...
inline SessionOptions& SessionOptions::AppendExecutionProvider_MACA(const OrtMACAProviderOptions& provider_options) {
  ThrowOnError(GetApi().SessionOptionsAppendExecutionProvider_MACA(p_, &provider_options));
  return *this;
}
...
inline OrtValue* CustomOpApi::KernelContext_GetOriOutput(OrtKernelContext* context, _In_ size_t index) {
  OrtValue* out;
  ThrowOnError(api_.KernelContext_GetOriOutput(context, index, &out));
  return out;
}
