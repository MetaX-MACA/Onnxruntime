...
class KernelDef {
 ...
 public:
  ...
  int GetInputDataLayout(int index) const {
    if(size_t(index) >= input_data_layout_.size()) {
      return 0;}
    return input_data_layout_[index];
  }

  int GetOutputDataLayout(int index) const {
    if(size_t(index) >= output_data_layout_.size()) {
      return 0;}
 ...
 private:
  ...
  // DataFormat
  std::vector<int> input_data_layout_;
  std::vector<int> output_data_layout_;
...
class KernelDefBuilder {
  ...
  */
  ...
  //set data layout
  KernelDefBuilder& SetInputDataLayout(const std::vector<int>& lay_out){
    kernel_def_->input_data_layout_ = lay_out;
    return *this;
  }
  KernelDefBuilder& SetOutputDataLayout(const std::vector<int>& lay_out){
    kernel_def_->output_data_layout_ = lay_out;
    return *this;
  }

  //get data layout
  int GetInputDataLayout(int index){
    return kernel_def_->GetInputDataLayout(index);
  }
  int GetOutputDataLayout(int index){
    return kernel_def_->GetOutputDataLayout(index);
  }
