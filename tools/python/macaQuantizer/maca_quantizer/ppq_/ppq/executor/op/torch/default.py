...
def convert_onnx_pads_to_torch(onnx_pads: List[int], mode: str=None, reverse=False) -> List[int]:
...
def Gather_forward(op: Operation, values: List[torch.Tensor], ctx: TorchBackendContext = None, **kwargs) -> torch.Tensor:
    ...
    if op.type == 'Gather':
        ...
        try:
            ...
            logger.error(f"IndexError: input_data shape: {input_data.shape}, array_idx: {array_idx}")
...
def Pad_forward(op: Operation, values: List[torch.Tensor], ctx: TorchBackendContext = None, **kwargs):
    ...
    pads = pads.tolist()
    pads = convert_onnx_pads_to_torch(pads, reverse=True)
