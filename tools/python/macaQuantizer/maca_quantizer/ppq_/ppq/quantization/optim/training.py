...
class TrainingBasedPass(QuantizationOptimizationPass):
    ...
    def split_graph_into_blocks(
        ...
        for op in graph.operations.values():
            ...
            if isinstance(op, QuantableOperation) and op.is_computing_op:
                ...
                # avoid block can not forward
                if not self.check_block(block):
                    for i in range(1, blocksize):
                        block = block_builder.build(op, blocksize-i)
    ...
    def check_block(self, block):
        ...
        if len(block.rps)<=2: 
            return flag
        for op in block.rps:
            if op.name in {block.sp.name, block.ep.name}: continue
            if len(op.inputs)==1: continue
            for var in op.inputs:
                # if var.is_parameter: continue
                if var.source_op is not None and var.source_op not in block.rps:
                    ppq_warning(f"{op.name} input \'{var.name}\' not in block")
                    flag=False
        return flag
...
class LearnedStepSizePass(TrainingBasedPass):
    ...
    * loss_fn(Callable)
    ...
    def finetune(
        ...
        if math.isclose(0.0, pre_loss,abs_tol=1e-5):
        ...
        for op in block.rps:
            ...
            for cfg, var in op.config_with_variable:
                if op.is_boundary and len(set(var.dest_ops)) > 1: continue
