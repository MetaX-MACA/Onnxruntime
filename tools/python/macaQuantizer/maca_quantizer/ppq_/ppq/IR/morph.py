...
class GraphFormatter(GraphCommandProcessor):
    ...
    def format_clip(self) -> None:
        ...
        for _, op in self.graph.operations.items():
            if op.type == 'Clip' and op.opset.onnx_opset_version() >= 11: 
                if op.inputs[1].shape is None and op.inputs[1].value is None:        
                    op.inputs[1].is_parameter = True
                    min_value = torch.finfo(torch.float32).min
                    op.inputs[1].value = convert_any_to_torch_tensor(min_value)

                if op.inputs[2].shape is None and op.inputs[2].value is None:
                    op.inputs[2].is_parameter = True
                    max_value = torch.finfo(torch.float32).max
                    op.inputs[2].value = convert_any_to_torch_tensor(max_value)
    ...
    def format_cast(self) -> None:
        ...
        for operation in interested_ops:
            ...
            if not isinstance(operation.attributes['to'], DataType):
    ...
    def remove_constant_input(self) -> None:
        ...
        for op in self.graph.operations.values():
            if op.type == 'Constant':
                ...
                # metax add: 输出节点Constant保留
                if op.outputs[0].name in self.graph.outputs:
                    continue
    ...
    def delete_isolated(self):
        ...
        while len(var_blacklist) > 0:
            ...
            for var in self.graph.variables.values():
                # metax add: 孤立输出节点保留
                if var.name in self.graph.outputs:
                    continue
...
class GraphMerger(GraphCommandProcessor):
    ...
    def _acceptable_command_types(self) -> List[GraphCommandType]:
        return [
            ...
            GraphCommandType.FUSE_GEMM,
            GraphCommandType.FUSE_BN,
            GraphCommandType.FUSE_CONV_MUL,
            GraphCommandType.FUSE_BIAS_ADD,
            GraphCommandType.FUSE_LAYERNORM,

        ]

    def process(self, command: GraphCommand) -> Any:
        if command.command_type == GraphCommandType.FUSE_GEMM:
            return self.fuse_gemm()
        if command.command_type == GraphCommandType.FUSE_BN:
            return self.fuse_bn()
        if command.command_type == GraphCommandType.FUSE_CONV_MUL:
            return self.fuse_conv_mul()
        if command.command_type == GraphCommandType.FUSE_BIAS_ADD:
            return self.fuse_bias_add()
        if command.command_type == GraphCommandType.FUSE_LAYERNORM:
            return self.fuse_layernorm()
    ...
    def fuse_layernorm(self, exclusive_search: bool = False):
        ...
        def _fuse(rm1: Operation, rm2: Operation, 
            ...
            fuse_op_type = 'LayerNorm' if int(os.environ.get("MXQ_USE_MACART_BACKEND", 1)) else 'LayerNormalization'
        
            layernorm_output_var.source_op.outputs.clear()
            layernorm = self.graph.create_operation(
                op_type = fuse_op_type,
    ...
    def fuse_conv_mul(self):
        """
            if mul multiplier is constant, merge mul to conv 
            Conv: wx + bias
            Mul: a * b 
            w_    = w * b
            bias_ = bias * b
        ...
        for pair in interested_pairs:
            ...
            if len(conv.inputs) > 2 and len(conv.inputs[2].value.shape) != 1: continue

            if len(const_value.shape)==0: #  constant single date 
                pass
            # elif len(const_value.shape)==1 and const_value.size != 1: # 1d constant mul can not merge
            ...
            self.graph.create_link_with_var(input_var, output_var)
        self.fuse_bias_add()
