...
def Resize_Socket(op: OperationBase) -> OpSocket:
    ...
    CHECK_OPSET(op=op, min_version_supported=10, max_version_supported=15, strict_check=True)
...
def Expand_Socket(op: OperationBase) -> OpSocket:
    ...
    CHECK_OPSET(op=op, min_version_supported=8, max_version_supported=16)
    in_plat  = [TargetPlatform.UNSPECIFIED, TargetPlatform.SOI]
    return OpSocket(
        op=op, in_plat=in_plat[: op.num_of_input],
        links=[VLink(in_idx=0, out_idx=0)])
...
def Ceil_Socket(op: OperationBase):
    pass


def CumSum_Socket(op: OperationBase):
    """
    From Opset 11 - 16:
...
def Einsun_Socket(op: OperationBase):
    """
    Inputs (1 - ∞)
...
def MaxUnpool_Socket(op: OperationBase):
    """
    Inputs (2 - 3)
        X : T1
...
def SequenceAt_Socket(op: OperationBase):
    """
    Inputs
        input_sequence : S
...
def SplitToSequence_Socket(op: OperationBase):
    """
    Inputs (1 - 2)
        input : T
...
def Xor_Socket(op: OperationBase):
    """
    Inputs
        A (non-differentiable) : T
    ...
    Outputs
        C (non-differentiable) : T1
    """
    CHECK_OPSET(op=op, min_version_supported=7, max_version_supported=16)
    return OpSocket(op=op)
...
DEFAULT_SOCKET_TABLE = {
    ...
    'ArgMin': DEFAULT_SOCKET_CREATOR,
    'Ceil': DEFAULT_SOCKET_CREATOR,
    'CumSum': CumSum_Socket,
    'Einsum': Einsun_Socket,
    'MaxUnpool': MaxUnpool_Socket,
    'SequenceAt': SequenceAt_Socket,
    'Xor': Xor_Socket
