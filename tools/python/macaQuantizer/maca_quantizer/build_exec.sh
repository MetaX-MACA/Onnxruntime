...
build_execute(){
  ...
  pyi-makespec -D   --specpath build/ --name "$EXEC_NAME" --python-option "$PythonOption"  --paths "$OldPath:$OldPath/maca_quantizer/ppq_:$OldPath/maca_quantizer/ppq_/ppq"  --hidden-import "$OldPath/maca_quantizer/ppq_/ppq"  \
                    --add-data  "$OldPath/maca_quantizer/ppq_/ppq/csrc/:ppq/csrc"  "$1"
