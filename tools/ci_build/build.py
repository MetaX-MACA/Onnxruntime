...
def parse_arguments():
    ...
    # MACA related
    parser.add_argument("--use_maca", action="store_true", help="Enable MACA.")
    parser.add_argument("--use_ppl", action="store_true", help="Enable PPL.")
    parser.add_argument("--use_dnn", action="store_true", help="Enable mcblas/mcdnn.")
    ...
    for unknown in unknowns:
        if unknown[0] != '-':
            pass
        elif unknown in ['--build_deb', '--build_rpm', '--package_dir']:
            pass
        else:
...
def generate_build_tree(
    ...
    cmake_args = [
        ...
        "-Donnxruntime_USE_PPL=" + ("ON" if args.use_ppl else "OFF"),
        "-Donnxruntime_USE_DNN=" + ("ON" if args.use_dnn else "OFF"),
    ...
    if args.use_maca:
        add_default_definition(cmake_extra_defines, "onnxruntime_USE_MACA", "ON")
    if args.use_cuda:
...
def build_maca_targets(args, make_maca_path, build_dir, configs, num_parallel_jobs, target=None):
    for config in configs:
        log.info("Building targets for %s configuration", config)
        build_dir2 = get_config_build_dir(build_dir, config)
        #cmd_args = [make_maca_path, '-k']
        cmd_args = [make_maca_path]
        if target:
            cmd_args.extend([target])
        if num_parallel_jobs != 1:
            cmd_args += ["-j{}".format(num_parallel_jobs)]
        #print(f'cmd_args: {cmd_args}, build_dir2: {build_dir2}')
        #exit()
        run_subprocess(cmd_args, cwd=build_dir2)
...
def setup_cuda_vars(args):
    ...
    if args.use_cuda:
        ...
        if not args.use_maca and (not cuda_home_valid or not cudnn_home_valid):
            raise BuildError(
...
def main():
    ...
    # to have cmake/ctest.
    if args.use_maca:
        cmake_path = f'{os.environ["CUCC_PATH"]}/tools/cmake_maca'
    else:
    ...
    if args.use_maca and (args.use_cuda or args.use_ppl):
        cuda_home = f'{os.environ["CUDA_PATH"]}'
        cudnn_home = f'{os.environ["CUDA_PATH"]}'
    ...
    if args.build:
        ...
        if args.use_maca:
            make_maca_path = f'{os.environ["CUCC_PATH"]}/tools/make_maca'
            build_maca_targets(args, make_maca_path, build_dir, configs, num_parallel_jobs, args.target)
        else:
