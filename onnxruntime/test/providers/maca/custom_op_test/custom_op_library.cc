...
__global__ void add_float(const float* a,const float* b,float* c, int64_t ele_num){
    int i = threadIdx.x;
    if(i<ele_num)
      {
        c[i] = b[i]+a[i];
...
__global__ void mul_float(const float* a, const float * b,float* c, int64_t ele_num){
    int i = threadIdx.x;
    if(i<ele_num)
      {
        c[i] = a[i] * b[0];
...
struct KernelOne {
  ...
  void Compute(OrtKernelContext* context) {
    ...
    // Do computation
#ifdef USE_MACA
    mcStream_t stream = reinterpret_cast<mcStream_t>(ort_.KernelContext_GetGPUComputeStream(context));
    add_float<<<1,size,1,stream>>>(X,Y,out,size);
#else
...
#ifdef USE_MACA
    mcStream_t stream = reinterpret_cast<mcStream_t>(ort_.KernelContext_GetGPUComputeStream(context));
    mul_float<<<1,size,1,stream>>>(X,Y,out,size);
#else
    for (int64_t i = 0; i < size; i++) {
      out[i] = X[i] * Y[0];
    }
...
struct CustomOpOne : Ort::CustomOpBase<CustomOpOne, KernelOne> {
  ...
  const char* GetName() const { return "custom_add"; };

#ifdef USE_MACA
  const char* GetExecutionProviderType() const { return "MACAExecutionProvider"; };
#else
...
struct CustomOpTwo : Ort::CustomOpBase<CustomOpTwo, KernelTwo> {
  void* CreateKernel(OrtApi api, const OrtKernelInfo* /* info */) const {
    ...
    const char* GetExecutionProviderType() const { return "CPUExecutionProvider"; };
  #endif
  const char* GetName() const { return "custom_mul"; };
