...
std::shared_ptr<TestCaseResult> TestCaseRequestContext::Run(PThreadPool tpool,
  ...
  if (concurrent_runs > 1 && data_count > 1) {
    ...
    if(repeat_count > 10){
      ctx.RunSequentially(10);
      struct timeval tv_begin, tv_end;
      gettimeofday(&tv_begin, NULL);
      ctx.RunSequentially(repeat_count - 10);
      gettimeofday(&tv_end, NULL);
      double cost_ms = (tv_end.tv_sec - tv_begin.tv_sec) * 1000.0 + (tv_end.tv_usec - tv_begin.tv_usec) / 1000.0;
      printf("\033[0;m");
      printf("\033[1;33m Run repeated %d times, average cost %f ms\n", (int)(repeat_count - 10), cost_ms / (repeat_count - 10));
      printf("\033[0;m");
    }
    else{
