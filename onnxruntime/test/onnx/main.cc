...
void usage() {
  printf(
      ...
      "\t-e [EXECUTION_PROVIDER]: EXECUTION_PROVIDER could be 'cpu', 'cuda', 'dnnl', 'tensorrt', 'maca', "
...
int real_main(int argc, char* argv[], Ort::Env& env) {
      ...
      switch (ch) {
        ...
        case 'e':
          if (!CompareCString(optarg, ORT_TSTR("cpu"))) {
            ...
            enable_rocm = true;
          } else if (!CompareCString(optarg, ORT_TSTR("maca"))) {
            enable_maca = true;
  ...
  std::vector<std::unique_ptr<ITestCase>> owned_tests;
  {
    ...
    if (enable_maca) {
      ...
      // TODO: Support arena configuration for users of test runner
      // sf.AppendExecutionProvider_ROCM(rocm_options);
      // sf.AppendExecutionProvider_MACA();
      OrtMACAProviderOptions maca_options;
      maca_options.do_copy_in_default_stream = true;
      // TODO: Support arena configuration for users of test runner
      sf.AppendExecutionProvider_MACA(maca_options);
#else
      fprintf(stderr, "MACA is not supported in this build");
      return -1;
#endif
    }
    if (enable_migraphx) {
