...
This op functions in much the same was as Dropout-11 and Dropout-13 do, execpt that the mask is output as a bit-packed uint32 tensor, instead of a boolean tensor.
  ...
  static const char* Swish_ver1_doc = R"DOC(
  Swish for macavx
  Swish(x) = x * sigmoid(x)
  )DOC";
  ONNX_CONTRIB_OPERATOR_SCHEMA(Swish)
      .SetDomain(kMetaxDomain)
      .SinceVersion(1)
      .SetDoc(Swish_ver1_doc)
      .Input(0, "data", "The input data as Tensor.", "T")
      .Output(0, "output", "The output.", "T")
  ...
  static const char* HardSwish_metax_doc = R"DOC(
  ...
  static const char* MultiHeadAttn_metax_doc = R"DOC(
  Multi HeadAttn V1: softmax(Q*K.T/sqrt(dim_dim))*V for per head.
  )DOC";

  ONNX_CONTRIB_OPERATOR_SCHEMA(MultiHeadAttentionV1)
      .SetDomain(kMetaxDomain)
      .SinceVersion(1)
      .SetDoc(MultiHeadAttn_metax_doc)
      .Attr("head_dim",
            "Attn head dims",
            AttributeProto::INT, static_cast<int64_t>(-1))
      .Attr("is_causal",
            "use causal mask or full mask",
            AttributeProto::INT, static_cast<int64_t>(0))
      .Attr("num_heads",
            "num heads of query",
            AttributeProto::INT, static_cast<int64_t>(-1))
      .Attr("num_kv_heads",
            "num heads of key and value",
            AttributeProto::INT, static_cast<int64_t>(-1))
      .AllowUncheckedAttributes()
      .Input(0,"query", "Input tensor","T")
      .Input(1,"key", "Input tensor","T")
      .Input(2,"value", "Input tensor","T")
      .Input(3, "attn_mask", "Input tensor", "T", OpSchema::Optional)
      .Output(0, "attn_out", "Output tensor", "T")
      .TypeConstraint("T", {"tensor(float16)", "tensor(float)"},
                      "Constrain input and output types to float tensors.")
      .TypeAndShapeInferenceFunction([](ONNX_NAMESPACE::InferenceContext& ctx) {
          propagateShapeAndTypeFromFirstInput(ctx);
    ...
    static const char* SpliAct_metax_doc = R"DOC(
      SplitAct impl USE for SD models:
          x, g = torch.split(X, [X.shape[-1]//2, X.shape[-1]//2], -1)
          OUTPUT = g * activation(x)
      Now, we just support 3 Dims.
      )DOC";

  ONNX_CONTRIB_OPERATOR_SCHEMA(SplitActivation)
      .SetDomain(kMetaxDomain)
      .SinceVersion(1)
      .SetDoc(SpliAct_metax_doc)
      .Attr("activation",
            "define activation type by string",
            AttributeProto::STRING, "")
      .AllowUncheckedAttributes()
      .Input(0,"input", "Input tensor","T")
      .Input(1, "bias", "Input tensor", "T", OpSchema::Optional)
      ...
      .TypeAndShapeInferenceFunction([](ONNX_NAMESPACE::InferenceContext& ctx) {
          ...
          for (int32_t i = 0; i < input_shape.dim_size(); i++) {
            input_dim_i = input_shape.dim(i);
            auto new_dim = outputs_shape.add_dim();
            if (i == input_shape.dim_size()-1) {
              new_dim->set_dim_value(input_dim_i.dim_value() / 2);
            } else {
              new_dim->set_dim_value(input_dim_i.dim_value());
            }
          }
          updateOutputShape(ctx, 0, outputs_shape);
  ...
  static const char* GroupNorm_metax_doc = R"DOC(
    group impl USE for SD models.
    )DOC";
  ONNX_CONTRIB_OPERATOR_SCHEMA(GroupNormalization)
      .SetDomain(kMetaxDomain)
      .SinceVersion(1)
      .SetDoc(GroupNorm_metax_doc)
      .Attr("num_groups",
            "channel group to norm",
            AttributeProto::INT, static_cast<int64_t>(-1))
      .Attr("epsilon",
            "epsilon",
            AttributeProto::FLOAT, 1e-5f)
      .AllowUncheckedAttributes()
      .Input(0,"input", "Input tensor","T")
      .Input(1, "scale", "Input tensor", "T")
      .Input(2, "bias", "Input tensor", "T")
      .Output(0, "output", "Output tensor", "T")
      .TypeConstraint("T", {"tensor(float16)", "tensor(float)"},
                      "Constrain input and output types to float tensors.")
      .TypeAndShapeInferenceFunction([](ONNX_NAMESPACE::InferenceContext& ctx) {
  ...
  static const char* Mish_ver1_doc = R"DOC(
  Mish for macavx
  ...
  static const char* QLinearRelu_ver1_doc = R"DOC(
  QLinearRelu
  )DOC";
  ONNX_CONTRIB_OPERATOR_SCHEMA(QLinearRelu)
      .SetDomain(kMetaxDomain)
      .SinceVersion(1)
      .SetDoc(QLinearRelu_ver1_doc)
      .Input(0, "X", "Input tensor", "T")
      .Input(1, "X_scale",
  ...
  ONNX_CONTRIB_OPERATOR_SCHEMA(LayerNorm)
      .SetDomain(kMetaxDomain)
      .SinceVersion(1)
      .SetDoc("LayerNormalization")
      .Attr("axis",
