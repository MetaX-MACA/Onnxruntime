...
Status QuickGeluFusion::ApplyImpl(Graph& graph, bool& modified, int graph_level, const logging::Logger& logger) const {
  ...
  for (auto node_index : node_topology_list) {
    ...
    if (graph_utils::IsSupportedOptypeVersionAndDomain(node, "Mul", {7, 13, 14}) &&
        ...
        (graph_utils::IsSupportedProvider(node, GetCompatibleExecutionProviders()) || node.GetExecutionProviderType().empty()) &&
    ...
    if (!graph_utils::IsSupportedOptypeVersionAndDomain(sigmoid_node, "Sigmoid", {6, 13}) ||
        !(graph_utils::IsSupportedProvider(sigmoid_node, GetCompatibleExecutionProviders()) || sigmoid_node.GetExecutionProviderType().empty()) ||
    ...
    if (!graph_utils::IsSupportedOptypeVersionAndDomain(mul_node, "Mul", {7, 13, 14}) ||
        !(graph_utils::IsSupportedProvider(mul_node, GetCompatibleExecutionProviders()) || sigmoid_node.GetExecutionProviderType().empty()) ||
