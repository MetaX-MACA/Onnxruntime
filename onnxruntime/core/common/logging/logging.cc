...
static const int MACART_LOG_LEVEL = parseENV("MACART_LOG_LEVEL", -1); // 
...
std::unique_ptr<Logger> LoggingManager::CreateLogger(const std::string& logger_id) {
  if(MACART_LOG_LEVEL>=0)
    return CreateLogger(logger_id, Severity(int(Severity::kFATAL) - std::min(MACART_LOG_LEVEL, int(Severity::kFATAL))), default_filter_user_data_, default_max_vlog_level_);
  else
