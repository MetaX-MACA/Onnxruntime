...
#ifdef USE_MACA
  if (provider_type == onnxruntime::kMacaExecutionProvider) {
    // Parallel execution mode does not support MACA EP
    if (session_options_.execution_mode != ExecutionMode::ORT_SEQUENTIAL) {
      LOGS(*session_logger_, WARNING)
          << "Parallel execution mode does not support the MACA Execution Provider. "
          << "So making the execution mode sequential for this session since it uses the MACA Execution Provider.";
...
#ifdef USE_MACA
  if (execution_providers_.Get(kMacaExecutionProvider) && execution_providers_.NumProviders() <= 2) {
    bool modified = false;
    ORT_RETURN_IF_ERROR_SESSIONID_(
        std::make_unique<InsertCastCustomNode>(&kernel_registry_manager)->Apply(graph, modified, *session_logger_));
