...
ProviderInfo_MACA* TryGetProviderInfo_MACA();
ProviderInfo_MACA& GetProviderInfo_MACA();
...
static ProviderLibrary s_library_maca("onnxruntime_pybind11_state" LIBRARY_EXTENSION
#ifndef _WIN32
                                      ,
                                      false /* unload - On Linux if we unload the cuda shared provider we crash */
#endif
...
std::shared_ptr<IExecutionProviderFactory> CreateExecutionProviderFactory_MACA(const OrtMACAProviderOptions& provider_options);
...
ProviderInfo_MACA* TryGetProviderInfo_MACA() try {
  return reinterpret_cast<ProviderInfo_MACA*>(s_library_maca.Get().GetInfo());
} catch (const std::exception& exception) {
  LOGS_DEFAULT(ERROR) << exception.what();
  return nullptr;
}
...
ProviderInfo_MACA& GetProviderInfo_MACA() {
  if (auto* info = TryGetProviderInfo_MACA())
    return *info;
...
ORT_API_STATUS_IMPL(OrtApis::SessionOptionsAppendExecutionProvider_MACA, _In_ OrtSessionOptions* options, _In_ const OrtMACAProviderOptions* maca_options) {
  API_IMPL_BEGIN
  auto factory = onnxruntime::CreateExecutionProviderFactory_MACA(*maca_options);
  if (!factory) {
    return OrtApis::CreateStatus(ORT_FAIL, "OrtSessionOptionsAppendExecutionProvider_MACA: Failed to load shared library");
  }
