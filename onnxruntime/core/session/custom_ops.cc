...
ORT_API_STATUS_IMPL(OrtApis::KernelContext_GetOriOutput, _Inout_ OrtKernelContext* context, _In_ size_t index, _Out_ OrtValue** out) {
  *out = reinterpret_cast<OrtValue*>(reinterpret_cast<onnxruntime::OpKernelContextInternal*>(context)->GetOutputMLValue(index));
  return nullptr;
};
...
common::Status CreateCustomRegistry(const std::vector<OrtCustomOpDomain*>& op_domains,
  ...
  for (const auto& domain : op_domains) {
    ...
    for (const auto* op : domain->custom_ops_) {
      auto input_count = op->GetInputTypeCount(op);
      std::vector<int> input_data_layout;
      for (size_t i = 0; i < input_count; i++) {
        input_data_layout.push_back(int(op->GetInputDataLayout(op, i)));
      }
  
      auto output_count = op->GetOutputTypeCount(op);
      std::vector<int> output_data_layout;
      for (size_t i = 0; i < output_count; i++) {
        output_data_layout.push_back(int(op->GetOutputDataLayout(op, i)));
      }
      KernelDefBuilder def_builder;
      def_builder.SetName(op->GetName(op))
          .SetDomain(domain->domain_)
          .SinceVersion(1)
          .SetInputDataLayout(input_data_layout)
          .SetOutputDataLayout(output_data_layout);
