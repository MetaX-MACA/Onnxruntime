...
ORT_API_STATUS_IMPL(OrtApis::SessionOptionsAppendExecutionProvider_MACA,
                    _In_ OrtSessionOptions* options, _In_ const OrtMACAProviderOptions* provider_options) {
  ORT_UNUSED_PARAMETER(options);
  ORT_UNUSED_PARAMETER(provider_options);
  return CreateNotEnabledStatus("MACA");
}
