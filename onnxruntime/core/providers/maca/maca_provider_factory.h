 ...
 * \param.
 */
ORT_API_STATUS(OrtSessionOptionsAppendExecutionProvider_MACA,
                // _In_ OrtSessionOptions* options);
                _In_ OrtSessionOptions* options, _In_ const OrtMACAProviderOptions* maca_options);
#ifdef __cplusplus
}
#endif
...
//   virtual bool CudaCall_true(int retCode, const char* exprString, const char* libName, int successCode, const char* msg) = 0;
  ...
  virtual int macaGetDeviceCount() = 0;
  virtual void MACAExecutionProviderInfo__FromProviderOptions(const onnxruntime::ProviderOptions& options, onnxruntime::MACAExecutionProviderInfo& info) = 0;
  ...
  virtual std::shared_ptr<onnxruntime::IExecutionProviderFactory> CreateExecutionProviderFactory(const OrtMACAProviderOptions& info) = 0;
  virtual std::shared_ptr<onnxruntime::IAllocator> CreateCudaAllocator(int16_t device_id, size_t gpu_mem_limit, int arena_extend_strategy,
