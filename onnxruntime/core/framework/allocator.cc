...
ORT_API_STATUS_IMPL(OrtApis::CreateMemoryInfo, _In_ const char* name1, enum OrtAllocatorType type, int id1,
  ...
  if (strcmp(name1, onnxruntime::CPU) == 0) {
    ...
    *out = new OrtMemoryInfo(
        ...
        id1, mem_type1);
  } else if (strcmp(name1, onnxruntime::MACA) == 0) {
    *out = new OrtMemoryInfo(
        onnxruntime::MACA, type, OrtDevice(OrtDevice::GPU, OrtDevice::MemType::DEFAULT, static_cast<OrtDevice::DeviceId>(id1)), id1,
        mem_type1);
  } else if (strcmp(name1, onnxruntime::MACA_PINNED) == 0) {
    *out = new OrtMemoryInfo(
        onnxruntime::MACA_PINNED, type, OrtDevice(OrtDevice::CPU, OrtDevice::MemType::CUDA_PINNED, static_cast<OrtDevice::DeviceId>(id1)),
        id1, mem_type1);
  }else {
