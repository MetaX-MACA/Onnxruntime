...
#ifdef USE_PPL
const MACAExecutionProviderInfo GetMacaExecutionProviderInfo(ProviderInfo_MACA* maca_provider_info,
                                                             const ProviderOptionsMap& provider_options_map) {
  ORT_ENFORCE(maca_provider_info);
  const auto it = provider_options_map.find(kMacaExecutionProvider);
  MACAExecutionProviderInfo info;
  if (it != provider_options_map.end())
    maca_provider_info->MACAExecutionProviderInfo__FromProviderOptions(it->second, info);
  else {
    info.device_id = cuda_device_id;
    info.gpu_mem_limit = gpu_mem_limit;
    info.arena_extend_strategy = arena_extend_strategy;
...
#endif
  } else if (type == kMacaExecutionProvider) {
#ifdef USE_PPL
  // if (auto* cuda_provider_info = TryGetProviderInfo_PPL()) {
  //   const CUDAExecutionProviderInfo info = GetCudaExecutionProviderInfo(cuda_provider_info,
  //                                                                           provider_options_map);
  //   external_allocator_info = info.external_allocator_info;

    OrtMACAProviderOptions maca_provider_info{};
    auto it = provider_options_map.find(type);
    if (it != provider_options_map.end()) {
        for (auto option : it->second) {
          if (option.first == "device_id") {
            if (!option.second.empty()) {
              maca_provider_info.device_id = std::stoi(option.second);
            } 
          }else{
            ORT_THROW("Invalid Maca EP option: ", option.first);
          }
        }
    }
  //   return cuda_provider_info->CreateExecutionProviderFactory(cuda_provider_info)->CreateProvider();
  if (std::shared_ptr<IExecutionProviderFactory> maca_provider_factory = onnxruntime::CreateExecutionProviderFactory_MACA(maca_provider_info)) {
      auto p = maca_provider_factory->CreateProvider();
      return p;
  } else {
    // if (!Env::Default().GetEnvironmentVar("CUDA_PATH").empty()) {
      ORT_THROW("MACA_PATH is set but MACA wasn't able to be loaded. Please install the correct version of MACA and mcDNN as mentioned in the GPU requirements page ");
    // }
  }
#endif
...
#ifdef USE_PPL
  m.def("getDeviceCount",[]() -> int32_t {
    return GetDeviceCount();
  },"get maca device count");
#endif
  m.def(
...
#if defined(USE_CUDA) || defined(USE_ROCM) || defined(USE_PPL)
  /*
...
void addObjectMethods(py::module& m, Environment& env, ExecutionProviderRegistrationFn ep_registration_fn) {
  ...
  device.def(py::init<OrtDevice::DeviceType, OrtDevice::MemoryType, OrtDevice::DeviceId>())
      ...
      .def_static("maca_pinned", []() { return OrtDevice::CPU; })
      .def_static("maca", []() { return OrtDevice::GPU; })
      .def_static("default_memory", []() { return OrtDevice::MemType::DEFAULT; })
      .def_static("maca_pinned_memory", []() { return OrtDevice::MemType::CUDA_PINNED; });
