...
void addOrtValueMethods(pybind11::module& m) {
  ...
  ortvalue_binding
...
#ifdef USE_PPL
        if(device.MemType() == OrtDevice::MemType::CUDA_PINNED){
          auto cpuMemcpy = [] (void* dst, const void* src, size_t num_bytes) -> void{
            memcpy(dst, const_cast<void*>(src), num_bytes);
          };
          CreateGenericMLValue(nullptr, GetMacaPinnedAllocator(device.Id()), "", array_on_cpu, ml_value.get(), true, false, cpuMemcpy);
        }else{
      ...
      // The tensor's memory is allocated on CUDA
        ...
        if (!IsCudaDeviceIdValid(logging::LoggingManager::DefaultLogger(), device.Id())) {
          throw std::runtime_error("The provided device id doesn't match any available MACA GPUs on the machine.");
        }
      ...
      throw std::runtime_error(
          ...
          "Please use the CUDA package of OnnxRuntime to use this feature.");
#endif
        } else if (strcmp(GetDeviceName(device), "maca") == 0) {
#ifdef USE_PPL
            if (!IsCudaDeviceIdValid(logging::LoggingManager::DefaultLogger(), device.Id())) {
              throw std::runtime_error("The provided device id doesn't match any available GPUs on the machine.");
            }
            allocator = GetCudaAllocator(device.Id());
        } else if(device.MemType() == OrtDevice::MemType::CUDA_PINNED){
            allocator = GetMacaPinnedAllocator(device.Id());
#else
      throw std::runtime_error(
          "Can't allocate memory on the MACA device using this package of OnnxRuntime. "
          "Please use the MACA package of OnnxRuntime to use this feature.");
#endif
        } else {
          throw std::runtime_error("Unsupported device: Cannot place the OrtValue on this device");
        }
