...
def get_ort_device_type(device_type, device_index):
    if device_type == "cuda":
        ...
        return C.OrtDevice.maca()
    elif device_type == "maca_pinned":
        return C.OrtDevice.maca_pinned()
...
def get_ort_memory_type(memory_type):
    if memory_type == "maca_pinned":
        return C.OrtDevice.maca_pinned_memory()
    else:
        return C.OrtDevice.default_memory()
...
class MultiDeviceInferenceSession():
    def __init__(self, model_path, providers, device_ids=None):
        ...
        if("MACAExecutionProvider" not in providers):
            device_ids = [0]
        if(device_ids is None):
            self.device_ids = [i for i in range(C.getDeviceCount())]
        else:
        ...
        for device_id in self.device_ids:
            ...
            self.session_dict[device_id] = InferenceSession(self.model_path, providers = providers, provider_options = provider_options)
    ...
    def run(self, output_names, input_dict,device_id=None):
        if(device_id is None):
            device_id = self.gainBestDeviceId()
    ...
    def bind_input(self, name, device_type, device_id, element_type, shape, buffer_ptr):
        ...
        self._iobinding.bind_input(
            ...
            C.OrtDevice(
                ...
                get_ort_memory_type(device_type),
                device_id,
            ),
            element_type,
            shape,
    ...
    def make(ort_device_name, device_id):
        return OrtDevice(
            C.OrtDevice(
                ...
                get_ort_memory_type(ort_device_name),
                device_id,
            )
        )
