    ...
    ReduceParam param)
{
    ...
    for (int64_t bx = blockIdx.x; bx < n_outer * num_block_inner; bx += grid_stride) {
        ...
        T val         = (T)op.InitVal();
        int64_t outer_off_write = out_idx * n_inner;
        ...
        for (int i = warp; i < 32; i += n_warps) {
            ...
            if (MultiBlock) {
                if (lane == 0 && i + inner < n_inner) {
                    ...
                    PPLAtomicWrite<dst_type, Operator>(op.dst + outer_off_write + i + inner, static_cast<dst_type>(val), op);
                }
            } else {
                if (lane == 0 && i + inner < n_inner) {
                    if (param == ReduceMean) {
                    ...
                    op.out(outer_off_write + i + inner, static_cast<dst_type>(val));
                }
            }
        }
    }
