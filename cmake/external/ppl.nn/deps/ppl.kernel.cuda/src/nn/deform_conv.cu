...
)
{
    ...
    //     output += padN;
    // }
    if(blockIdx.x >= channel || threadIdx.x >= padN) return;
    T value = bias[blockIdx.x];
    int offset = blockIdx.x*padN + threadIdx.x;
    output[offset] = value;
}
...
#undef FAKE_GEMM_PARAM
    ...
    __half *columns       = reinterpret_cast<__half *>(temp_buffer);
    __half *trans_columns = columns + group * col_trans_in_h * col_trans_in_w;
    __half *biasBuffer    = trans_columns + group * N * K;
    ...
    for (int64_t b = 0; b < batch / n_parallel_imgs; ++b) {
        ...
        for (int g = 0; g < group; g++) {
            ...
            __half *tmp_a      = (__half *)flt + g * oc_per_grp * col_trans_out_w;
            __half *tmp_b      = (__half *)trans_columns + g * col_trans_out_h * col_trans_out_w;
            __half *tmp_output = (__half *)output + b * out_c * col_trans_out_h + g * oc_per_grp * col_trans_out_h;
            __half *tmp_bias = (__half *)bias + g * oc_per_grp;
            if(tmp_bias){
                pad_bias<half><<<oc_per_grp,col_trans_out_h,0,stream>>>(tmp_bias,biasBuffer,col_trans_out_h,oc_per_grp);
            }else{
                biasBuffer = nullptr;
            }
            algo_param_t algo_param;
            algo_param.UseDefaultF1Kernel(device_prop);
            PPLCUDAGemmForwardImp(
                device_prop, stream, function, &shape_a, tmp_a, &shape_b, tmp_b, biasBuffer, &shape_c, tmp_output, gemm_param, temp_buffer, fuse_param, algo_param);
        }
    }
    return ppl::common::RC_SUCCESS;
}