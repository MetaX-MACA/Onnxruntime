...
template<typename T>
__global__ void ReverseData(const T* input, T*output,int n){
  __shared__ T s_data[1024];
  int inOffset = blockDim.x*blockIdx.x;
  int in = inOffset + threadIdx.x;
  if(in < n){
    s_data[threadIdx.x] = input[in];
  }
  __syncthreads();
  
  if(in <n){
      output[n - 1 - in] = s_data[threadIdx.x];
  }
}

template<typename T>
__global__ void shlData(T*input,T*output,int n){
  __shared__ T s_data[1024];
    int inOffset = blockDim.x*blockIdx.x;
  int in = inOffset + threadIdx.x;
  if(in == 0){
  ...
  if(in < n){
    s_data[threadIdx.x] = input[in - 1];
  }
  __syncthreads();
  
  if(in < n){
    output[in] = s_data[threadIdx.x];
  }
}
    ...
    void* output)
{
  ...
  if(row_size == num_elems) {       \
    ...
    cudaError_t err = cudaMalloc((void**)&part_sum, (part_num)*sizeof(TYPE));            \
    if(err!=cudaSuccess){      \
        printf("cuda malloc failed:%d\n",err);      \
    }      \
    TYPE *inputTmp = nullptr;      \
    TYPE *outputTmp = nullptr;      \
    if(reverse)      \
    {      \
      cudaMalloc((void**)&inputTmp, (num_elems)*sizeof(TYPE));      \
      cudaMalloc((void**)&outputTmp,(num_elems)*sizeof(TYPE));      \
      ReverseData<TYPE><<<part_size,1024,0,stream>>>((const TYPE*)input,inputTmp,num_elems);      \
      if(exclusive){      \
        shlData<TYPE><<<part_num, 1024, 0, stream>>>((TYPE*)inputTmp,(TYPE*)input,num_elems);      \
      }else{      \
        input = inputTmp;      \
      }      \
    }else{      \
        if(exclusive){      \
          cudaMalloc((void**)&inputTmp, (num_elems)*sizeof(TYPE));      \
          shlData<TYPE><<<part_num, 1024, 0, stream>>>((TYPE*)input,inputTmp,num_elems);      \
          input = inputTmp;      \
        }      \
    }      \
    ...
    if(reverse){      \
      ScanWithBaseSumSinglePass<TYPE><<<part_num, 1024, 0, stream>>>(      \
        (const TYPE*)input, (TYPE*)part_sum, (TYPE*)outputTmp, num_elems, part_size);      \  
      ReverseData<TYPE><<<part_num,1024,0,stream>>>((const TYPE*)outputTmp,(TYPE*)output,num_elems);      \
    }else{      \
      ScanWithBaseSumSinglePass<TYPE><<<part_num, 1024, 0, stream>>>(               \
