...
__global__ void radixSort(
    ...
    unsigned int *prefixData)
{
    __shared__ unsigned int s_cnt[SORT_RADIX_SIZE * BLK_SORT_SIZE / 64];
    ...
    if (threadIdx.x < SORT_RADIX_SIZE * BLK_SORT_SIZE / 64)
        s_cnt[threadIdx.x] = 0;
    __syncthreads();

    int lane        = threadIdx.x & 63;
    int warpId      = threadIdx.x >> 6;
    int activeWarps = (blockIdx.x == gridDim.x - 1) ? DivUp((size & (BLK_SORT_SIZE - 1)), 64) : BLK_SORT_SIZE / 64;
    if (activeWarps == 0)
        activeWarps = 16;
    int64_t tid                     = blockIdx.x * blockDim.x + threadIdx.x;
    bool inRange                    = tid < size;
    uint64_t active             = __ballot_sync(0xffffffffffffffff, inRange);
    const uint64_t lane_mask_lt = ~(0xffffffffffffffff << (lane));
    if (tid - lane < size) {
    ...
    #else
        keyRadix = bfe(intKey, bitPos, SORT_RADIX_BITS);
    #endif
        int radixPrefix = 0;
        for (int i = dir * (SORT_RADIX_SIZE - 1);
            ...
            uint64_t ballot = __ballot_sync(active, flag);
            int warpCnt         = __popcll(ballot);
            int lanePrefix      = __popcll(ballot & lane_mask_lt);//线程束中，当前线程之前有多少线程活跃
            int warpPrefix      = 0;
            if (inRange && lane == 0) {
                s_cnt[i * BLK_SORT_SIZE / 64 + warpId] = warpCnt;
            }
            __syncthreads();

            // prefix sum in warp
            if (threadIdx.x < 16) {
                warpCnt             = s_cnt[i * BLK_SORT_SIZE / 64 + threadIdx.x];
                unsigned int prefix = warpCnt;
                for (int j = 1; j < 16; j <<= 1) {
                    warpCnt = __shfl_up_sync(0xffffffffffffffff, prefix, j, 16);
                    if (threadIdx.x >= j) {
                        prefix += warpCnt;
                    }
                }
                s_cnt[i * BLK_SORT_SIZE / 64 + threadIdx.x] = prefix;
            }
            __syncthreads();
            if (inRange && warpId > 0) {
                warpPrefix = s_cnt[i * BLK_SORT_SIZE / 64 + warpId - 1];
            }
...
__global__ void prefixSum(
    ...
    T *blkTotal)
{
    __shared__ T warp_cnt[SIZE_PER_SCAN >> 6];
    ...
    for (int iterOff = 0; iterOff < blkScanSize; iterOff += SIZE_PER_SCAN) {
        ...
        lanePrefix = data;
        for (int i = 1; i < 64; i <<= 1) {
            data = __shfl_up_sync(0xffffffffffffffff, lanePrefix, i, 64);
            if (lane >= i) {
                lanePrefix += data;
            }
        }
        ...
        T warpPrefix = 0;
        if (threadIdx.x < (SIZE_PER_SCAN >> 6)) {
            data  = warp_cnt[threadIdx.x];
            T sum = data;
            for (int i = 1; i < 16; i <<= 1) {
                data = __shfl_up_sync(0xffffffffffffffff, sum, i, 16);
                if (threadIdx.x >= i) {
                    sum += data;
                }
            }
        ...
        subScanPrefix += warp_cnt[(SIZE_PER_SCAN >> 6) - 1];
    }
    // blk scan total
    if (threadIdx.x == 0) {
    ...
    unsigned int bitPos)
{
    ...
    if (threadIdx.x < SORT_RADIX_SIZE * BLK_SORT_SIZE / 64)
        s_cnt[threadIdx.x] = 0;
    __syncthreads();
    int activeWarps = (blockIdx.x == gridDim.x - 1) ? DivUp((size & (BLK_SORT_SIZE - 1)), 63) : BLK_SORT_SIZE / 64;
    if (activeWarps == 0)
        activeWarps = 16;
    ...
    if (tid - lane < size) {
        ...
        for (int i = dir * (SORT_RADIX_SIZE - 1);
            ...
            bool flag               = inRange && (keyRadix == i);
            uint64_t ballot     = __ballot_sync(active, flag);
            unsigned int lanePrefix = __popcll(lane_mask_lt & ballot);
            unsigned int warpCnt    = __popcll(ballot);
