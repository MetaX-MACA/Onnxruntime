...
__global__ void __launch_bounds__(CTA_SIZE_IN_THD * 2) KERNEL_NAME(SPK_KPARAM_LIST)
...
#pragma unroll
    ...
    uint tid = threadIdx.x % 256;
    uint maca_tid = threadIdx.x;
    uint tid_wrapper = (maca_tid - maca_tid / 64) * 32;

    uint local_tid = tid & 0x1f;

    uint set_tid = tid & (SET_SIZE_IN_THD - 1);
    uint maca_set_tid = maca_tid & (256 - 1);

    uint set_id = (maca_tid >> (SET_SIZE_IN_BITS + 1)) & 0x7;

    //uint set_widx = (set_tid >> WARP_SIZE_IN_BITS) & (SET_SIZE_X_IN_WARP - 1);
    //uint set_widy = set_tid >> (WARP_SIZE_IN_BITS + SET_SIZE_X_IN_BITS);
    uint set_widx = (maca_set_tid >> (WARP_SIZE_IN_BITS + 1)) & (SET_SIZE_X_IN_WARP - 1);
    uint set_widy = maca_set_tid >> (WARP_SIZE_IN_BITS + 1 + SET_SIZE_X_IN_BITS);
    ...
    uint dCv4_idy = cta_idy * TILE_M_V1_PER_CTA +
                    (maca_tid - (maca_tid / 64) * 32) / TILE_N_V8_PER_CTA;

    uint dCv4_idx = cta_idx * TILE_N_V8_PER_CTA +
                    maca_tid % TILE_N_V8_PER_CTA;

    bool dCv4_x_valid = (dCv4_idx < num_flt_per_grp_pad_v8) & (((maca_tid - (maca_tid / 64) * 32) / TILE_N_V8_PER_CTA) < TILE_M_PER_CTA);
    ...
    uint mma_read_idx = maca_tid % TILE_N_V8_PER_CTA;
    uint mma_read_idy = (maca_tid - (maca_tid / 64) * 32) / TILE_N_V8_PER_CTA;
...
#pragma unroll
    ...
    const int lane_id = __lane_id() % 32;
    int offset = (lane_id / 4) * 32 + ((((set_id * TILE_K_V8_PER_SET) & 0x7) * _INT4_TO_4INT_) ^ lane_id);
    sAv1_read = set_widy * TILE_M_PER_WARP * TILE_K_V2_PER_CTA + offset;
    sBv1_read = set_widx * TILE_N_PER_WARP * TILE_K_V2_PER_CTA + SM_A_V1_1BUF * BUF_NUM + offset;
...
#pragma unroll
    ...
    READ_sAv1(db0_sAv1, sm_base_v1, sAv1_read);
    READ_sBv1(db0_sBv1, sm_base_v1, sBv1_read);
...
#if TILE_K_PER_SET == 16 || TILE_K_PER_SET == 32
        READ_sAv1(db1_sAv1, sm_base_v1, sAv1_read);
        READ_sBv1(db1_sBv1, sm_base_v1, sBv1_read);
