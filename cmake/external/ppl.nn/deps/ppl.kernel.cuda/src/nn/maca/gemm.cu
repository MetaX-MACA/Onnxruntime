    ...
    const algo_param_t &algo_param)
{
    ...
    int4 *input0_tmp = (int4 *)input;
    if (transA == 1) {
        dim3 grid(DivUp(K_pad, 32), DivUp(M_pad, 32), 1);
        dim3 block(32, 32, 1);
        if (type == ppl::common::DATATYPE_FLOAT32) {
            matrix_transpose<float><<<grid, block, 0, stream>>>((float *)temp_buffer, (float *)input, 1.f, K_pad, M_pad);
        } else if (type == ppl::common::DATATYPE_FLOAT16) {
            matrix_transpose<__half><<<grid, block, 0, stream>>>((__half *)temp_buffer, (__half *)input, 1.f, K_pad, M_pad);
        } else {
            return ppl::common::RC_UNSUPPORTED;
        }
        input0_tmp = (int4 *)temp_buffer;
