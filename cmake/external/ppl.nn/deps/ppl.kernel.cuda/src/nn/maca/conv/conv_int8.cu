	    ...
	    uint64_t workspace)
{
    ...
    for(unsigned int spk = 0; spk < 1; spk++) {
        ...
        for(unsigned int kid = 0; kid < g_int8_kvec.size(); kid++) {
            ...
            block_size.x = g_int8_kvec[kid].cta_size_in_thd * 2;
            block_size.y = 1;
            block_size.z = 1;
	        ...
	        if(elapsed < minTime){
                algo_param.algo_name = g_int8_kvec[kid].kname;
                algo_param.kid = kid;
    ...
    bool spk_only)
{
    ...
    if (num_chl_per_grp <= 64 && !spk_only) {
        ...
        if(nominees.size() == 0) {
            ...
            //For maca we need 2x blocksize
            nominee.SetIdxnKernelParam(128, 64, k_per_step, 64, 32, k_per_step, flt_pad_size, 256, 4096, 1, 1, mma_shape);
        ...
        if(estimate_cta_num <= sm_num || spk_only) {
            ...
            if(nominees.size() == 0) {
                ...
                //For maca we need change 128-threads to 256-threads
                nominee.Set2spkKernelParam(64, 64, 64, 32, 32, 64, flt_size, 1, 256, 16384, 1, 1, mma_shape);
                nominees.push_back(std::make_pair(nominee, 0.f));
            }
            ...
            if(nominees.size() == 0) {
                ...
                //For maca, we need change 128-threads to 256-threads
                nominee.SetSwzlKernelParam(128, 128, 64, 64, 64, flt_size, 1, 256, 16384, 1, 1, mma_shape);
                nominees.push_back(std::make_pair(nominee, 0.f));
            }
        }
    }
