...
__device__ inline int __dp4a(int srcA, int srcB, int c)
{
    int8_t  *pA = (int8_t*)&srcA;
    int8_t  *pB = (int8_t*)&srcB;
    ...
    algo_param_t &algo_param)
{
    ...
    if (transA == 1) { // input is shape of (K, M), we need K as the 1st inner dim
        ...
        dim3 grid(DivUp(K_pad_v4, 32), DivUp(M_pad, 32), 1);
        dim3 block(32, 32, 1);
        int8_matrix_transpose<<<grid, block, 0, stream>>>((int*)temp_buffer, (int*)input, K_pad_v4, M_pad);
    ...
    const algo_param_t &algo_param)
{
    ...
    int tile_n_per_cta  = g_int8_kvec[kid].tile_n_per_cta;
    int tile_k_per_cta  = g_int8_kvec[kid].tile_k_per_cta;
    int cta_size_in_thd = g_int8_kvec[kid].cta_size_in_thd * 2;
    int smem_size       = g_int8_kvec[kid].smem_size;
#endif
    dim3 block_size, grid_size;
    ...
    for (int thd_off = tid; thd_off < BLK_TILE_N_V4; thd_off += BLK_SIZE) {
        ...
        if (in_output_range) {
            ...
            if (fuse_param.has_activation)
                activation<float>(fuse_param.has_activation, out);
            if (fuse_param.has_clip)
                clip<float>(out, fuse_param.clip_min, fuse_param.clip_max);
            int concatV4_off = 0;
