...
//#if __CUDACC_VER_MAJOR__ * 1000 + __CUDACC_VER_MINOR__ * 10 >= 9020
    ...
    for (unsigned int spk = 0; spk < 1; spk++) {
        ...
        for (unsigned int kid = 0; kid < g_fp16_kvec.size(); kid++) {
            ...
            block_size.x = g_fp16_kvec[kid].cta_size_in_thd * 2;
            block_size.y = 1;
            block_size.z = 1;
    ...
    bool spk_only)
{
    ...
    if (num_chl_per_grp <= 32 && !spk_only) {
        ...
        if(nominees.size() == 0) {
            ...
            //We change 128-thread per block to 256 per block
        ...
        if(estimate_cta_num <= sm_num || spk_only) {
            ...
            if(nominees.size() == 0) {
                ...
                //We change 256-thread per block to 512 per block
                nominee.Set2spkKernelParam(64, 64, 64, 32, 32, 32, flt_size, 1, 512, 16384, 1, 1, mma_shape);
                nominees.push_back(std::make_pair(nominee, 0.f));
            }
            ...
            if(nominees.size() == 0) {
                ...
                //We change 128-thread per block to 256 per block
                nominee.SetSwzlKernelParam(128, 128, 16, 64, 64, flt_size, 1, 256, 8192, 1, 1, mma_shape);
                nominees.push_back(std::make_pair(nominee, 0.f));
            }
        }
    }
