...
#pragma unroll
    ...
    uint tid_x       = (((laneId >> 4) << 2) ^ 0xf) >> 2;
    uint tid_y       = (laneId & 0xf) >> 1;

    uint store_tid_x = tid & 0x3;
    uint store_tid_y = (tid & 0x1f) >> 2;

    uint warp_idx = (threadIdx.x >> (WARP_SIZE_IN_BITS + 1)) & (CTA_SIZE_X_IN_WARP - 1);
    uint warp_idy = threadIdx.x >> (WARP_SIZE_IN_BITS + 1 + CTA_SIZE_X_IN_BITS);
...
#pragma unroll
    ...
    __shared__ int4 sm_base_v4[(SM_IN_ID_SIZE + SM_IN_OFF_SIZE) * 2];
    ...
    {
        SET_IN_Mv1_ID(tid, sm_base_v4);
        SET_IN_Mv1_ID(tid + SM_IN_ID_SIZE + SM_IN_OFF_SIZE, sm_base_v4);
    }
#else
...
#pragma unroll
    ...
    {
        SET_IN_Mv1_ID(tid + CTA_SIZE_IN_THD * i, sm_base_v4);
        SET_IN_Mv1_ID(tid + CTA_SIZE_IN_THD * i + SM_IN_ID_SIZE + SM_IN_OFF_SIZE, sm_base_v4);
    }
#endif
    ...
    {
        SET_IN_Kv16_OFF(tid, sm_base_v4);
        SET_IN_Kv16_OFF(tid + SM_IN_ID_SIZE + SM_IN_OFF_SIZE, sm_base_v4);
    }
    ...
    {
        for (int c_idx = 0; c_idx < C_ITEMS_PER_THD / 2; ++c_idx) {
            int x = C[2 * c_idx];
            int y = C[2 * c_idx + 1];
            int x0 = __shfl_down_sync(0xffffffffffffffffull, x, (laneId / 4) * 8 + (laneId % 4) * 4 - laneId);
            int y0 = __shfl_down_sync(0xffffffffffffffffull, x, (laneId / 4) * 8 + (laneId % 4) * 4 + 2 - laneId);
            int x1 =
                __shfl_down_sync(0xffffffffffffffffull, y, ((laneId - 4) / 4) * 8 + ((laneId - 4) % 4) * 4 - laneId);
            int y1 = __shfl_down_sync(0xffffffffffffffffull, y,
                                      ((laneId - 4) / 4) * 8 + ((laneId - 4) % 4) * 4 + 2 - laneId);
            if ((laneId / 4) % 2 == 0) {
                C[2 * c_idx] = x0;
                C[2 * c_idx + 1] = y0;
            } else {
