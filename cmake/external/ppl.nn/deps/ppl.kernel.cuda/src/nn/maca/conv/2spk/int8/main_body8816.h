...
#pragma unroll
    ...
    uint local_tid = tid & 0x3f;

    uint set_id    = (tid >> SET_SIZE_IN_BITS) & 0x7;

    uint set_widx  = (tid >>  WARP_SIZE_IN_BITS) & (SET_SIZE_X_IN_WARP - 1);
    uint set_widy  =  tid >> (WARP_SIZE_IN_BITS  +  SET_SIZE_X_IN_BITS);
    ...
    uint mma_idx    = (local_tid >> 4) << 1;
    uint mma_idy    = local_tid & 0xf;
    ...
    uint sRv4_read  = (mma_read_idy / TILE_M_PER_MMA_QTR)  * TILE_N_V4_PER_CTA    * TILE_M_PER_MMA_QTR  +
                    ...
                    (smem_inter_off)         * TILE_N_V4_PER_MMA +
                       smem_intra_off;
...
*/
    ...
    //Still has 2-way bank conflict
    unsigned offset  = (local_tid & 0xf) * TILE_K_V4_PER_CTA
                        + (((((local_tid >> 4) << 2) ^ 0xf) >> 2) ^ (((local_tid & 0x7) >> 1) << 2));
    // unsigned offset  = (local_tid & 0xf) * TILE_K_V4_PER_CTA
    //                     + (((((local_tid >> 4) << 2) ^ 0xf) >> 2));
    uint sAv1_read   = set_widy * TILE_M_PER_WARP * TILE_K_V4_PER_CTA + offset;
    uint sBv1_read   = set_widx * TILE_N_PER_WARP * TILE_K_V4_PER_CTA + SM_A_V1_1BUF * BUF_NUM + offset;
