...
#define HADD2_INST(_d, _a, _b)                                      \
    {                                                               \
        __half2* a = reinterpret_cast<__half2*>(&_a);               \
        __half2* b = reinterpret_cast<__half2*>(&_b);               \
        __half a_low = __low2half(*a);                              \
        __half a_high = __high2half(*a);                            \
        __half b_low = __low2half(*b);                              \
        __half b_high = __high2half(*b);                            \
        __half2 d = __halves2half2(a_low + b_low, a_high + b_high); \
...
// only test for HMAX2_INST(_d,_a,0,_a)
#define HMAX2_INST(_d, _a, _b, _c)                     \
    {                                                  \
        __half2* ha = reinterpret_cast<__half2*>(&_a); \
        __half2 hd;                                    \
        hd.x = ha->x > __half(0) ? ha->x : __half(0);  \
        hd.y = ha->y > __half(0) ? ha->y : __half(0);  \
        _d = *reinterpret_cast<int*>(&hd);             \
    }
...
// only test for HMIN2_INST(_d,_a,_b,_a)
#define HMIN2_INST(_d, _a, _b, _c)                     \
    {                                                  \
        __half2* ha = reinterpret_cast<__half2*>(&_a); \
        __half2* hb = reinterpret_cast<__half2*>(&_b); \
        __half2 hd;                                    \
        hd.x = ha->x < hb->x ? ha->x : hb->x;          \
        hd.y = ha->y < hb->y ? ha->y : hb->y;          \
        _d = *reinterpret_cast<int*>(&hd);             \
    }
