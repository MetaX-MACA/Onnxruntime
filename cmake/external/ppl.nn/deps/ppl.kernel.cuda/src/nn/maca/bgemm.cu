...
template<typename T>
__global__ void matrix_transpose_opt1(
    T *output,
    T *input,
    const int in_height,
    const int in_width,
    const int out_height,
    const int out_width,
    const int inImageSize,
    const int outImageSize
) 
{
    int bx = blockIdx.x;
    int by = blockIdx.y << 4;
    ...
    int ty = threadIdx.y << 4;
   	...
   	int col_idx = bx * blockDim.x + threadIdx.x;
	int row_idx = by * blockDim.y + ty;
    int in_idx = bz * inImageSize + row_idx * in_width + col_idx;
    int out_idx = bz * outImageSize +  col_idx * out_width + row_idx;
    
    T tmpbuffer[16];
    T reg_zero = 0;
    int valid_height = in_height - row_idx;
    int real_width = min(16,out_width - row_idx);
    int i = 0, index = in_idx;        

    #pragma unroll
    for(; i < real_width&&col_idx < in_width; i += 1,index += in_width)
    {
        tmpbuffer[i] = i < valid_height ? input[index] : reg_zero;
    }
    
    #pragma unroll
    for(i = 0, index = out_idx; i < real_width&&col_idx < in_width; i++, index++)
    {
        output[index] = tmpbuffer[i];
    }
}

template<typename T>
__global__ void matrix_transpose_opt2(
    T *output,
    T *input,
    const int in_height,
    const int in_width,
...
) 
{
    ...
    int bz = blockIdx.z;
   	...
   	int col_idx = bx * blockDim.x + threadIdx.x;
    int row_idx = by * blockDim.y + threadIdx.y;

    int in_idx = bz * inImageSize + row_idx * in_width + col_idx;
    int out_idx = bz * outImageSize +  col_idx * out_width + row_idx;
   
	if( row_idx < out_width && col_idx < in_width)
	{
        T reg_zero = 0;
        T tmp = row_idx < in_height ? input[in_idx] : reg_zero;
		output[out_idx] = tmp;
	}
}
    ...
    const GemmKernelParam *param)
{
    ...
    const int inSize = dim0*dim1;
    const int outSize = dim1*dim0_pad;
...
#else// !VERSION1
#define TRANSWEIGHT(Type)                                                                                      \
    matrix_transpose_opt1<Type><<<grid,block,0,stream>>>((Type *)tmp_weight, (Type *)weight, dim0, dim1, dim1, dim0_pad,inSize,outSize);
    dim3 grid(DivUp(dim1, 16), DivUp(dim0_pad, 256), batch);
    dim3 block(16, 16, 1); 
#endif//VERSION1
    ...
    algo_param_t &algo_param)
{
    ...
    for (unsigned int kid = 0; kid < g_fp16_kvec.size(); kid++) {
        ...
        block_size.x = cta_size_in_thd * 2;
        block_size.y = 1;
        block_size.z = 1;
    ...
    const algo_param_t &algo_param)
{
    ...
    int tile_n_per_cta  = g_fp16_kvec[kid].tile_n_per_cta;
    int tile_k_per_cta  = g_fp16_kvec[kid].tile_k_per_cta;
    int cta_size_in_thd = g_fp16_kvec[kid].cta_size_in_thd * 2;
    int smem_size       = g_fp16_kvec[kid].smem_size;
#endif
    dim3 block_size, grid_size;
