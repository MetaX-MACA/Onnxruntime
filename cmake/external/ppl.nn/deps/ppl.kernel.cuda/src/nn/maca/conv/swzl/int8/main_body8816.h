...
#pragma unroll
    ...
    uint sts_idx = (tid & 0x3); // we not consider bank conflict now
    uint sts_idy   =   tid >> 2;
...
#pragma unroll
   ...
   // CVT_SM_PTR(smp_base_v1, sm_base_v1);
    ...
    const int offset = ((lane_id & 0xf) >> 1) * TILE_K_V4_PER_CTA + ((((lane_id >> 4) << 2) ^ 0xf) >> 2);
    sAv1_read = warp_idy * TILE_M_PER_WARP * TILE_K_V4_PER_CTA + offset;
    sBv1_read = warp_idx * TILE_N_PER_WARP * TILE_K_V4_PER_CTA + SM_A_V1_1BUF * BUF_NUM + offset;
...
#pragma unroll
    ...
    const int shfl_offset0 = (lane_id / 4) * 8 + (lane_id % 4) * 4 - lane_id;
    const int shfl_offset1 = ((lane_id - 4) / 4) * 8 + ((lane_id - 4) % 4) * 4 - lane_id;
    for (int c_idx = 0; c_idx < C_ITEMS_PER_THD / 2; ++c_idx) {
        int x = C[2 * c_idx];
        int y = C[2 * c_idx + 1];
        int x0 = __shfl_down_sync(0xffffffffffffffffull, x, shfl_offset0);
        int y0 = __shfl_down_sync(0xffffffffffffffffull, x, shfl_offset0 + 2);
        int x1 = __shfl_down_sync(0xffffffffffffffffull, y, shfl_offset1);
        int y1 = __shfl_down_sync(0xffffffffffffffffull, y, shfl_offset1 + 2);
        if ((lane_id / 4) % 2 == 0) {
            C[2 * c_idx] = x0;
            C[2 * c_idx + 1] = y0;
        } else {
