...
#include "cudakernel/memory/space_to_depth.h"
...
ppl::common::RetCode PPLCUDASpaceToDepthForwardImp(
    cudaStream_t stream,
    SpaceToDepthKernelParam param,
    const ppl::common::TensorShape* input_shape,
    const void* input,
    const ppl::common::TensorShape* output_shape,
    void* output)
{
    int num_transpose_dim = 6; // from sapce_to_depth definition
    TransposeKernelParam trans_param;
    /*
    trans_param.perm[0] = 0;
    trans_param.perm[1] = 3;
    trans_param.perm[2] = 5;
    trans_param.perm[3] = 1;
    trans_param.perm[4] = 2;
    trans_param.perm[5] = 4;
    */
    trans_param.perm.push_back(0);
    trans_param.perm.push_back(3);
    trans_param.perm.push_back(5);
    trans_param.perm.push_back(1);
    trans_param.perm.push_back(2);
    trans_param.perm.push_back(4);
    ...
    input_shape_trans.SetDim(2, input_shape->GetDim(2) / param.blocksize);
    int trans_channel = input_shape->GetDim(1) / param.blocksize / param.blocksize;
    input_shape_trans.SetDim(3, param.blocksize );
    input_shape_trans.SetDim(4, input_shape->GetDim(3) / param.blocksize);
    input_shape_trans.SetDim(5,  param.blocksize);
