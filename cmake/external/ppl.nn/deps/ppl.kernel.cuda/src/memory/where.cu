...
template <typename X, typename Y, BroadcastType C_BType, BroadcastType X_BType, BroadcastType Y_BType>
__global__ void ppl_cukernel_where_complex_broadcast_diff(
    int64_t num_elems,
    int num_dims,
    GArray<int64_t> condition_strides,
    const bool* condition,
    ...
    float* output)
{
    const X* input_x_local = static_cast<const X*>(input_x);
    const Y* input_y_local = static_cast<const Y*>(input_y);
    ...
    float out_val     = condition[c_index] ? (float)input_x_local[x_index] : (float)input_y_local[y_index];
    output[index] = out_val;
}
    ...
    void* output)
{
    ...
    int block_size = 256;
    int grid_size  = (num_elems + block_size - 1) / block_size;
    ppl::common::datatype_t data_type_x = input_x_shape->GetDataType();
    ppl::common::datatype_t data_type_y = input_y_shape->GetDataType();
...
#define SWITCH_CASE(TYPE)                                                                                                                                                                          \
    ...
    if(ppl::common::GetSizeOfDataType(output_shape->GetDataType()) == ppl::common::GetSizeOfDataType(input_x_shape->GetDataType()) && ppl::common::GetSizeOfDataType(output_shape->GetDataType()) == ppl::common::GetSizeOfDataType(input_y_shape->GetDataType())){
        switch (ppl::common::GetSizeOfDataType(output_shape->GetDataType())) {
            SWITCH_CASE(int8_t);
            SWITCH_CASE(int16_t);
            SWITCH_CASE(int32_t);
...
#define HANDLE_Y(data_btype_x, data_btype_y)                                                                                                                                                                \
    switch (data_btype_y)                                                                                                                                                                                   \
    {                                                                                                                                                                                                       \
    case ppl::common::DATATYPE_FLOAT32:                                                                                                                                                                     \
        ppl_cukernel_where_complex_broadcast_diff<data_btype_x, float, C_BType, X_BType, Y_BType>                                                                                                           \
            <<<grid_size, block_size, 0, stream>>>(                                                                                                                                                         \
                num_elems, num_dims, condition_strides, (const bool*)condition, input_x_strides, (const void*)input_x, input_y_strides, (const void*)input_y, output_strides_fast, (float*)output);         \
        return ppl::common::RC_SUCCESS;                                                                                                                                                                     \
        break;                                                                                                                                                                                              \
    case ppl::common::DATATYPE_FLOAT16:                                                                                                                                                                     \
        ppl_cukernel_where_complex_broadcast_diff<data_btype_x, half, C_BType, X_BType, Y_BType>                                                                                                            \
            <<<grid_size, block_size, 0, stream>>>(                                                                                                                                                         \
                num_elems, num_dims, condition_strides, (const bool*)condition, input_x_strides, (const void*)input_x, input_y_strides, (const void*)input_y, output_strides_fast, (float*)output);         \
        return ppl::common::RC_SUCCESS;                                                                                                                                                                     \
        break;                                                                                                                                                                                              \
    default:                                                                                                                                                                                                \
        ...
        {
        case ppl::common::DATATYPE_FLOAT32:
            HANDLE_Y(float, data_type_y)
            break;
        case ppl::common::DATATYPE_FLOAT16:
            HANDLE_Y(half, data_type_y)
            break;
        default:
            break;
        }
    ...
    switch (input_y_btype) {                                                                                                                                                                                              \
        case BroadcastType::NoBroadcast: {                                                                                                                                                                                \
            return WhereForwardImpComplexBroadcast<condition_btype,                                                                                                                                                       \
                                                   input_x_btype,                                                                                                                                                         \
                                                   BroadcastType::NoBroadcast>(stream, condition_strides, condition, input_x_strides, input_x, input_y_strides, input_y, output_strides_fast, output_shape, input_x_shape, input_y_shape, output);      \
            break;                                                                                                                                                                                                        \
        }                                                                                                                                                                                                                 \
        case BroadcastType::SimpleBroadcast: {                                                                                                                                                                            \
            return WhereForwardImpComplexBroadcast<condition_btype,                                                                                                                                                       \
                                                   input_x_btype,                                                                                                                                                         \
                                                   BroadcastType::SimpleBroadcast>(stream, condition_strides, condition, input_x_strides, input_x, input_y_strides, input_y, output_strides_fast, output_shape, input_x_shape, input_y_shape, output);  \
            break;                                                                                                                                                                                                        \
        }                                                                                                                                                                                                                 \
        case BroadcastType::ComplexBroadcast: {                                                                                                                                                                           \
            return WhereForwardImpComplexBroadcast<condition_btype,                                                                                                                                                       \
                                                   input_x_btype,                                                                                                                                                         \
                                                   BroadcastType::ComplexBroadcast>(stream, condition_strides, condition, input_x_strides, input_x, input_y_strides, input_y, output_strides_fast, output_shape, input_x_shape, input_y_shape, output); \
            break;                                                                                                                                                                                                        \
        }                                                                                                                                                                                                                 \
    }
