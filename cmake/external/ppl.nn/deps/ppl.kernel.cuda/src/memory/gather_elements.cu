    ...
    const void* indices)
{
    ...
    int64_t indices_idx = get_indices_val(indices_element_size, index, indices, input_axis_size);
...
#define SWITCH_CASE(TYPE)                                                                                                                                                                                                       \
    case sizeof(TYPE): {                                                                                                                                                                                                        \
        ppl_cukernel_gather_elements<<<grid_size, block_size, 0, stream>>>(num_elems, input_axis_size, output_outer_block_fast, output_axis_size, output_inner_block_fast, (const TYPE*)input, (TYPE*)output, indices_element_size, (const void*)indices); \
