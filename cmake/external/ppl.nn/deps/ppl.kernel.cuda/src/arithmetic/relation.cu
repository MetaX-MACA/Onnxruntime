...
template <>
__device__ inline bool ppl_relation_scalar<Relation_Equal, int32_t>(int32_t a, int32_t b)
{
    return a == b;
}
template <>
__device__ inline bool ppl_relation_scalar<Relation_Greater, int32_t>(int32_t a, int32_t b)
{
    return a > b;
}
template <>
__device__ inline bool ppl_relation_scalar<Relation_Greater_Or_Equal, int32_t>(int32_t a, int32_t b)
{
    return a >= b;
}
template <>
__device__ inline bool ppl_relation_scalar<Relation_Less, int32_t>(int32_t a, int32_t b)
{
    return a < b;
}
    ...
    if (num_broadcast_dims == 0) {
        ...
        ppl_cukernel_relation_naive<op_type, T><<<grid_size,
                                                  ...
                                                  stream>>>(num_elems,
                                                            ...
                                                            (const T *)input1,
                                                            (bool *)output);
    } else if (num_broadcast_dims == dim_count && false) { // Error case(mcErrorIllegalAddress): input0: [1,77], input1: [77, 1], output: [77, 77]
        uint64_t grid_size = (num_elems + block_size - 1) / block_size;
    ...
    {                                                                                             \
        if (input_shape0->GetDataType() == ppl::common::DATATYPE_FLOAT16) {                       \
            ...
            return PPLCUDARelationForwardImp<Relation_##OPTYPE, int64_t>(stream,                  \
                                                                         ...
                                                                         output_shape,            \
                                                                         output);                 \
        } else if (input_shape0->GetDataType() == ppl::common::DATATYPE_INT32) {                  \
            return PPLCUDARelationForwardImp<Relation_##OPTYPE, int32_t>(stream,                  \
                                                                         input_shape0,            \
