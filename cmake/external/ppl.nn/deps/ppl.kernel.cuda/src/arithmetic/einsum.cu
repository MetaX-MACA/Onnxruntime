...
template <typename T>
__global__ void ppl_cukernel_einsum_ii_i(const T* input,T* output,uint64_t i, uint64_t j,uint64_t k){
    int index_j = threadIdx.x + blockDim.x * blockIdx.x;
    int index_i = threadIdx.y + blockDim.y * blockIdx.y;
    if(index_i >= i || index_j >= j) return;
    int id = index_i*j + index_j;
    int srcid = index_i*k + index_j*j + index_j;
    output[id] = input[srcid];
}
...
ppl::common::RetCode PPLCUDAEinSum_ii_i_ForwardImp(
    cudaStream_t stream,
...
{
    // ii -> i
    auto i = output_shape->GetDim(0);
    auto j = output_shape->GetDim(1);
    auto k = j*j;
    ...
    switch(datatype){
        case ppl::common::DATATYPE_FLOAT32:{
            ppl_cukernel_einsum_ii_i<float><<<grid, block, 0, stream>>>((const float*)input, (float*)output, i, j, k);
            break;
        }
        case ppl::common::DATATYPE_FLOAT16:{
            ppl_cukernel_einsum_ii_i<half><<<grid, block, 0, stream>>>((const half*)input, (half*)output, i, j, k);
            break;
        }
        case ppl::common::DATATYPE_INT64:{
            ppl_cukernel_einsum_ii_i<int64_t><<<grid, block, 0, stream>>>((const int64_t*)input, (int64_t*)output, i, j, k);
            break;
        }
        case ppl::common::DATATYPE_FLOAT64:{
            ppl_cukernel_einsum_ii_i<double><<<grid, block, 0, stream>>>((const double*)input, (double*)output, i, j, k);
            break;
        }
        default:
            return ppl::common::RC_UNSUPPORTED;
