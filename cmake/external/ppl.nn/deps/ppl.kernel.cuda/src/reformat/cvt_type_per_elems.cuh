...
template <>
__device__ inline void cuda_kernel_cvt_per_elems<UINT8_FLOAT16>(const void* input, int in_offset, void* output, int out_offset, ReFormatParam param)
{
    float i_step = param.i_step;
    //((__half*)output)[out_offset] = (__half)_uint82float(((uint8_t*)input)[in_offset], i_step, param.i_zero_point);
    ((__half*)output)[out_offset] = (__half)((uint8_t*)input)[in_offset];
}
template <>
...
template <>
__device__ inline void cuda_kernel_cvt_per_elems<FLOAT16_INT64>(const void* input, int in_offset, void* output, int out_offset, ReFormatParam param)
{
    ((int64_t*)output)[out_offset] = ((half*)input)[in_offset];
}
...
template <>
__device__ inline void cuda_kernel_set_zero_per_elems<FLOAT16_INT64>(void* output, int out_offset)
{
    ((int64_t*)output)[out_offset] = 0;
}