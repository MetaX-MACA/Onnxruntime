...
template <typename T>
__device__ bool __less(T& lhs, T& rhs) {
    return lhs < rhs;
}

template<>
__device__ bool __less<half>(half& lhs, half& rhs) {
#if __CUDA_ARCH__ >= 600 && __CUDACC_VER_MAJOR__ >= 9
...
template <typename T>
__global__ void ppl_argmax_select_last(
    PPLReduceDimDes des,
    const T* input,
    int64_t* output)
{
...
template <typename T>
__global__ void ppl_argmax_select_first(
    PPLReduceDimDes des,
    const T* input,
    int64_t* output)
{
    ...
    int32_t select_last_index)
{
    ...
    if (input_shape->GetDataType() == ppl::common::DATATYPE_FLOAT16) {
        if(select_last_index==1)
            ppl_argmax_select_last<half><<<grid_dim, block_dim, 0, stream>>>(des, (const half*)input, (int64_t*)output);
        else
            ppl_argmax_select_first<half><<<grid_dim, block_dim, 0, stream>>>(des, (const half*)input, (int64_t*)output);

    } else if (input_shape->GetDataType() == ppl::common::DATATYPE_FLOAT32) {
        if(select_last_index==1)
            ppl_argmax_select_last<float><<<grid_dim, block_dim, 0, stream>>>(des, (const float*)input, (int64_t*)output);
        else
            ppl_argmax_select_first<float><<<grid_dim, block_dim, 0, stream>>>(des, (const float*)input, (int64_t*)output);
    }
    else if (input_shape->GetDataType() == ppl::common::DATATYPE_INT8) {
        if(select_last_index==1)
            ppl_argmax_select_last<int8_t><<<grid_dim, block_dim, 0, stream>>>(des, (const int8_t*)input, (int64_t*)output);
        else
            ppl_argmax_select_first<int8_t><<<grid_dim, block_dim, 0, stream>>>(des, (const int8_t*)input, (int64_t*)output);
    }
    else if (input_shape->GetDataType() == ppl::common::DATATYPE_INT32) {
        if(select_last_index==1)
            ppl_argmax_select_last<int32_t><<<grid_dim, block_dim, 0, stream>>>(des, (const int32_t*)input, (int64_t*)output);
        else
            ppl_argmax_select_first<int32_t><<<grid_dim, block_dim, 0, stream>>>(des, (const int32_t*)input, (int64_t*)output);
    }
    else {
