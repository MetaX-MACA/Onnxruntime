...
struct fuse_param_t {
    int has_activation = 0; // 1: relu,  2: sigmoid, 3: HardSigmoid, 4: Swish, 5: HardSwish
    bool has_clip      = false;
    float clip_min;
    float clip_max;
    int has_prelu = 0;
    ...
    bool has_elt = false;
    void* pre_data;
    int has_elt_activation = 0; // 1: relu,  2: sigmoid, 3: HardSigmoid, 4: Swish, 5: HardSwish
    ...
    float act_beta = 0.0f;
    float elt_act_alpha = 0.0f;
    float elt_act_beta = 0.0f;
};
...
struct fuse_info_t {
    ...
    std::vector<int32_t> output_channels; // save for parell conv output
};
