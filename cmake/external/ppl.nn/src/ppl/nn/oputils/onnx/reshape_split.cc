...
ppl::common::RetCode ReshapeSplit(InputOutputInfo* info, const ir::Attr* arg, const int64_t axis)
{
    const TensorShape& shape = *info->GetInput<TensorImpl>(0)->GetShape();
    int dim_count = shape.GetDimCount();
    uint32_t split_axis = axis >= 0 ? axis : axis + dim_count;
    if(shape.GetDim(split_axis) % info->GetOutputCount() != 0){
        LOG(DEBUG) << "ERROR: input dim[" << split_axis << "]:" << shape.GetDim(split_axis) << " mod " << "ouput num: " 
                   << info->GetOutputCount()<<" != 0";
        return RC_INVALID_VALUE;
    }
    for (uint32_t i = 0; i < info->GetOutputCount(); ++i) {
        std::vector<int64_t> tmp_output_dim(dim_count);
        for (int it = 0; it < dim_count; ++it) {
            tmp_output_dim[it] = (it == split_axis) ? (shape.GetDim(it) / info->GetOutputCount()): shape.GetDim(it);
        }
...
RetCode ReshapeSplit(InputOutputInfo* info, const ir::Attr* arg) {
    ...
    if (info->GetInputCount() == 1) {
        ...
        return ReshapeSplit(info, arg, param->axis);
    }
