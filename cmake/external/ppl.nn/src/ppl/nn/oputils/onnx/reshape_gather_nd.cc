...
RetCode ReshapeGatherND(InputOutputInfo* info, const ir::Attr*arg) {
    ...
    if (last_indices_dim < 1 || last_indices_dim > (r - param->batch_dims)) {
        LOG(DEBUG) << "ERROR: last index dim[" << last_indices_dim << "] is out of range[1, " << (r - param->batch_dims) << "].";
        return RC_INVALID_VALUE;
    }
    const uint32_t output_dim_count = q + r - last_indices_dim - 1 - param->batch_dims;
    ...
    for (; i < output_dim_count; i++) {
        output->SetDim(i, input_data->GetDim(i - (q - 1) + last_indices_dim + param->batch_dims));
    }
    output->CalcPadding();
    return RC_SUCCESS;
}
