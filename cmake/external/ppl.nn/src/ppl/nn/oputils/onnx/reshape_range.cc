...
common::RetCode ReshapeRange(InputOutputInfo* info, const ir::Attr*) {
    ...
    if (data_type == DATATYPE_INT64) {
        ...
        start = *info->GetInput<TensorImpl>(0)->GetBufferPtr<int32_t>();
        limit = *info->GetInput<TensorImpl>(1)->GetBufferPtr<int32_t>();
        delta = *info->GetInput<TensorImpl>(2)->GetBufferPtr<int32_t>();
