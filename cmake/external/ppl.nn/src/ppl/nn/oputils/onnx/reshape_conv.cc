...
RetCode ReshapeConv(InputOutputInfo* info, const ir::Attr* arg) {
    ...
    for (int32_t i = 0; i < kernel_dims; ++i) {
        ...
        {
            out_dim = (int64_t)ceilf((float)x->GetDim(j) / (float)param->strides[i]);
        }
        else
        {
            out_dim = (x->GetDim(j) + param->pads[i] + param->pads[i + kernel_dims] - kernel_shape_eff) / param->strides[i] + 1;
        }
        if (out_dim <= 0) {
