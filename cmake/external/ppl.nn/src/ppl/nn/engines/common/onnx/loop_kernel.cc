...
static RetCode UpdateSubgraphInputs(int64_t trip_count, RuntimeImpl* subgraph,
    ...
    #endif
    #if 0
        printf("xxxxxxxxxxxxxxxxx dest(%d) datatype %s\n", i, GetDataTypeStr(dst->GetShape()->GetDataType()));
        if(dst->GetShape()->GetDataType() == DATATYPE_INT64){
            int64_t* host_data = (int64_t*)malloc(dst->GetShape()->CalcBytesIncludingPadding());
            status = dst->CopyToHost(host_data);
            if (status != RC_SUCCESS) {
                LOG(ERROR) << "get int64 failed: " << GetRetCodeStr(status);
                return status;
            }
            char file_name[128];
            sprintf(file_name, "./input_%d.txt", i);
            FILE *fd = fopen(file_name, "w");
            for(int j = 0; j < dst->GetShape()->CalcElementsIncludingPadding(); j++){
                fprintf(fd, "%ld\n", host_data[j]);
            }
            fclose(fd);
        } else if(dst->GetShape()->GetDataType() == DATATYPE_INT32){
            int32_t* host_data = (int32_t*)malloc(dst->GetShape()->CalcBytesIncludingPadding());
            status = dst->CopyToHost(host_data);
            if (status != RC_SUCCESS) {
                LOG(ERROR) << "get int64 failed: " << GetRetCodeStr(status);
                return status;
            ...
            fclose(fd);
        } else if(dst->GetShape()->GetDataType() == DATATYPE_FLOAT32){
            float* host_data = (float*)malloc(dst->GetShape()->CalcBytesIncludingPadding());
            status = dst->CopyToHost(host_data);
            if (status != RC_SUCCESS) {
                LOG(ERROR) << "get int64 failed: " << GetRetCodeStr(status);
                return status;
            ...
            fclose(fd);
        } else if(dst->GetShape()->GetDataType() == DATATYPE_FLOAT16){
            unsigned short* host_data = (unsigned short*)malloc(dst->GetShape()->CalcBytesIncludingPadding());
            status = dst->CopyToHost(host_data);
            if (status != RC_SUCCESS) {
                LOG(ERROR) << "get int64 failed: " << GetRetCodeStr(status);
                return status;
...
static RetCode SetOutputsFromSubgraph(const LoopInfo& info, Device* kernel_dev, Device* tmp_cpu_device,
    ...
    for (uint32_t i = info.loop_carried_dep_num; i < ctx->GetOutputCount(); ++i) {
        auto dst = ctx->GetOutput<TensorImpl>(i);
        dst->SetDevice(kernel_dev);
        auto& outputs = info.intermediate_outputs[i - info.loop_carried_dep_num];
...
RetCode LoopKernel::DoExecute(KernelExecContext* ctx) {
    ...
    char *env_ptr = getenv("LOOP_TIMES");
    if(env_ptr != nullptr){
        loop_times = atoi(env_ptr);
    }
    while (trip_count < max_trip_count && keep_going && trip_count < loop_times) {
        //printf("~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ loop run cnt %d\n", trip_count);
        if (trip_count != 0) {
