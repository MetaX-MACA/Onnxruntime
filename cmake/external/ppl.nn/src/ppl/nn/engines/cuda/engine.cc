...
RetCode CudaEngine::DoOptimize(const utils::SharedResource& resource, ir::Graph* graph, RuntimePartitionInfo* info) {
    ...
    if(options_.io_data_format){
        std::unordered_map<std::string, ppl::common::dataformat_t> io_data_format = *static_cast<std::unordered_map<std::string, ppl::common::dataformat_t>*>(options_.io_data_format);
        for(auto p : io_data_format){
            edgeid_t io_edge_id = graph->topo->GetInput(p.first);
            if(io_edge_id == INVALID_EDGEID){
                io_edge_id = graph->topo->GetOutput(p.first);
            }
            if(io_edge_id != INVALID_EDGEID){
                auto iter = graph->data->shapes.find(io_edge_id);
                if(iter != graph->data->shapes.end()){
                    iter->second.data_format = p.second;
                }else{
                    LOG(ERROR) << "Shape is not exists; Edge " << p.first << " set data_format failed";
                }
            }
        }
    };
...
ppl::common::RetCode CudaEngine::SetQuantParamInfo(CudaEngine* engine, va_list args) {
    char* qp_info = va_arg(args, char*);
    engine->cuda_flags_.quant_info = *(QuantParamInfo*)qp_info;;
    return RC_SUCCESS;
}
...
CudaEngine::ConfHandlerFunc CudaEngine::conf_handlers_[] = {
    ...
    CudaEngine::SetQuantParamInfo, // ENGINE_CONF_SET_QUANT_PARAM_INFO
};
