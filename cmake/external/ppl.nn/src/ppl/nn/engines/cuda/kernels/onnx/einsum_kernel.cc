...
// template<typename T>
// void printDebugInfo(const void * srcBuffer , TensorShape *shape,cudaStream_t &stream_){
//     vector<char> tmpdata;
//     auto size = shape->CalcElementsIncludingPadding();
//     tmpdata.resize(size*sizeof(T));
//     // tensor->CopyToHost(tmpdata.data());
//     cudaMemcpyAsync((void *)tmpdata.data(), srcBuffer , size*sizeof(T), cudaMemcpyDeviceToHost, stream_);
//     char * buffer = (char*)tmpdata.data();
//     printf("======dim======\n");
//     for(int i = 0; i < shape->GetDimCount(); i++)
//     {
//         printf("%d,",shape->GetDim(i));
//     }
//     printf("\n");
//     printf("============data==========\n");
//     for(int i = 0; i < size; i++){
//         if(sizeof(T) == 2){
//             half *tmp = (half*)(buffer + i*sizeof(T));
...
ppl::common::RetCode EinSumKernel::MatMulImpl(KernelExecContext* ctx) {
    ...
    if(weight->GetShape()->GetDataType() == 5){
        newshape = *weight->GetShape();
    }else if(weight->GetShape()->GetDataType() == 7 ){
        newshape = *weight->GetShape();
        newshape.SetDataType(ppl::common::DATATYPE_FLOAT16);
    }
    
    {
        ...
        if(weight->GetShape()->GetDataType() == 7){
            BufferDesc tmpbuffer;
            auto tmpshape = *weight->GetShape();
            tmpshape.SetDataType(ppl::common::DATATYPE_FLOAT16);
            status = GetCudaDevice()->Realloc(tmpshape,&tmpbuffer);
            if (status != ppl::common::RC_SUCCESS) {
                LOG(ERROR) << "alloc buffer for constant failed: " << GetRetCodeStr(status);
                return status;
            }
            PPLCUDACastForwardImp(stream, weight->GetShape(),weight->GetBufferPtr(),&tmpshape,tmpbuffer.addr,0);
            PPLCUDABgemmModifyWeights(stream, &newshape, tmpbuffer.addr, weight_buffer.addr,
                                  &param_kernel_);
            ppl::common::Destructor __tmp_buffer__([this, &tmpbuffer]() -> void {
                GetCudaDevice()->Free(&tmpbuffer);
            });
        }else{
    ...
    if (is_input0_pad) {
        if(newshape0.GetDataType() == 7){
            newshape0.SetDataType(ppl::common::DATATYPE_FLOAT16);
            BufferDesc tmpbuffer;
            status = GetCudaDevice()->Realloc(newshape,&tmpbuffer);
            if (status != ppl::common::RC_SUCCESS) {
                LOG(ERROR) << "alloc buffer for constant failed: " << GetRetCodeStr(status);
                return status;
            }
            auto stream = GetStream();
            PPLCUDACastForwardImp(stream, input0->GetShape(),input0->GetBufferPtr(),&newshape0,tmpbuffer.addr,0);
            ...
            PPLCUDABgemmPadInput(stream, &newshape0, tmpbuffer.addr, input0_buffer.addr, &param_kernel_);
        ...
        if(newshape0.GetDataType() == 7){
            ...
            PPLCUDACastForwardImp(stream, input0->GetShape(),input0->GetBufferPtr(),&newshape0,input0_buffer.addr,0);
            bmm_input0 = input0_buffer.addr;
        }else{
    ...
    void* bgemm_out;
    if (is_output_pad) {
        if(newshape_out.GetDataType() == 7){
            newshape_out.SetDataType(ppl::common::DATATYPE_FLOAT16);
        }
    ...
    if(shape_in0.GetDataType() == ppl::common::DATATYPE_FLOAT64 ){
        auto out_shape = *output->GetShape();
        out_shape.SetDataType(ppl::common::DATATYPE_FLOAT16);
        BufferDesc tmpbuffer;
        status = GetCudaDevice()->Realloc(out_shape,&tmpbuffer);
        if (status != ppl::common::RC_SUCCESS) {
            LOG(ERROR) << "alloc buffer for constant failed: " << GetRetCodeStr(status);
            return status;
        }
        status = PPLCUDABgemmForwardImp(GetCudaDevice()->GetDeviceProp(), stream, module_func, &newshape0, bmm_input0,
                                        &newshape, weight_buffer.addr,&out_shape, tmpbuffer.addr,
        ...
        if(is_output_pad){
            PPLCUDABgemmCvtOutput(stream, &out_shape, bgemm_out, tmpbuffer.addr);
            PPLCUDACastForwardImp(stream, &out_shape, bgemm_out, output->GetShape(),output->GetBufferPtr(),0);
        }else{
            PPLCUDACastForwardImp(stream, &out_shape,tmpbuffer.addr,output->GetShape(),output->GetBufferPtr(),0);
        }
    } else if (shape_in0.GetDataType() == ppl::common::DATATYPE_FLOAT16 ) {
...
ppl::common::RetCode EinSumKernel::DoExecute(KernelExecContext* ctx) {
    ...
    if(ctx->GetInputCount()==1 && equation == "...ii ->...i"
        && input_shape0->GetDimCount() == 3){
        status = PPLCUDAEinSum_ii_i_ForwardImp(GetStream(),input_shape0,input0,output_shape,output,equation);
        return status;
    }
