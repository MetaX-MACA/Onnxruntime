...
ppl::common::RetCode SwishKernel::DoExecute(KernelExecContext* ctx) {
    ...
    ppl::common::RetCode status =
        PPLCUDAUnarySwishForwardImp(GetStream(), input->GetShape(), input->GetBufferPtr(), output->GetShape(),
                                        output->GetBufferPtr(), param_->beta, input_quant.scale[0], output_quant.scale[0]);
    return status;
}
