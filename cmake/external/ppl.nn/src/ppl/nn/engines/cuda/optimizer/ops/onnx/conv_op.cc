...
ConvOp::ConvOp(const ir::Node* node) : CudaOptKernel(node) {
    ...
    infer_type_func_ = [this](InputOutputInfo* info, std::vector<CudaTensorQuant>* quant, datatype_t type) -> RetCode {
        if(info->GetInput<TensorImpl>(1)->GetShape()->GetDimCount() == 5){
    ...
    infer_dims_func_ = [this](InputOutputInfo* info) -> RetCode {
        ...
        if (inshape->GetDimCount() != 3 && inshape->GetDimCount() != 4 && inshape->GetDimCount() != 5) {
        ...
        if (param_.extra_param.fuse_info.output_channels.empty() == false){
            info->GetOutput<TensorImpl>(0)->GetShape()->SetDim(1, param_.extra_param.fuse_info.output_channels[0]);
            for(size_t i=1; i<info->GetOutputCount(); i++){
                auto out_shape = info->GetOutput<TensorImpl>(i);
                out_shape->GetShape()->Reshape(info->GetInput<TensorImpl>(0)->GetShape()->GetDims(),
                                           info->GetInput<TensorImpl>(0)->GetShape()->GetRealDimCount());
                out_shape->GetShape()->SetDim(1, param_.extra_param.fuse_info.output_channels[i]);
            }
        }
    ...
    infer_unsafe_dims_func_ = [this](InputOutputInfo* info, std::set<uint32_t>* illegal_inputs) -> RetCode {
        ...
        int64_t k = shape1.GetDim(0);
        if (shape0.GetDimCount() == 1 && shape0.GetDim(0) == 0){
            // shape0.Reshape({k, 128});
            for (uint32_t i = 0; i < info->GetOutputCount(); ++i) {
                if(shape1.GetDimCount() == 4)
                    info->GetOutput<TensorImpl>(i)->GetShape()->Reshape({1, k, 224, 224});
                else if(shape1.GetDimCount() == 3)
                    info->GetOutput<TensorImpl>(i)->GetShape()->Reshape({1, k, 224});
                else if(shape1.GetDimCount() == 5)
                    info->GetOutput<TensorImpl>(i)->GetShape()->Reshape({1, k, 224, 224, 224});
            }
        } else{
            for (uint32_t i = 0; i < info->GetOutputCount(); ++i) {
                info->GetOutput<TensorImpl>(i)->GetShape()->Reshape(shape0.GetDims(), shape0.GetRealDimCount());
            }
        }
        return ppl::common::RC_SUCCESS;
    };
...
KernelImpl* ConvOp::CreateKernelImpl() const {
    ...
    else if (param_.extra_param.algo_info.algo_type == "TuringFMMAImpgemm") {
        return CreateKernelImplWithParam<ConvFmmaKernel>(&param_);
    }
#endif
    return nullptr;
}
