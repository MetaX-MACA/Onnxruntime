...
public:
    ConvImmaKernel(const ir::Node* node) : CudaKernel(node) {
        ...
        quant_buffer_1_ = nullptr;
        quant_buffer_2_ = nullptr;
    #endif
    }
    ...
    ~ConvImmaKernel(){
        ...
        if(quant_buffer_1_ != nullptr){
            cudaFree(quant_buffer_1_);
        }
        if(quant_buffer_2_ != nullptr){
            cudaFree(quant_buffer_2_);
        }
    }
#endif
...
private:
    ...
    ppl::common::RetCode DoExecute_MCDNN(KernelExecContext*);
    ppl::common::RetCode DoExecute(KernelExecContext*) override;
#if defined(PPLNN_USE_DNN) && defined(PPLNN_USE_MACA)
    ppl::common::RetCode SetActivationParam(bool is_conv3d, fuse_param_t *param_ptr, bool is_act1, cudnnActivationDescriptor_t activation_desc);
#endif
private:
...
//#if 1
    ...
    uint32_t conv_bias_dequant_ = 0;
    void* quant_buffer_1_;
    uint32_t quant_buffer_1_len_ = 0;
    void* quant_buffer_2_;
    uint32_t quant_buffer_2_len_ = 0;
#endif
};
