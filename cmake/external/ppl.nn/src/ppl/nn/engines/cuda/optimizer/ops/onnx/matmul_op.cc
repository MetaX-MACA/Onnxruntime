...
MatMulOp::MatMulOp(const ir::Node* node) : CudaOptKernel(node) {
    infer_type_func_ = [](InputOutputInfo* info, std::vector<CudaTensorQuant>* quant, datatype_t type) -> RetCode {
        if (type != DATATYPE_INT8) {
            ...
            if(info->GetInput<TensorImpl>(0)->GetShape()->GetDimCount() >= 3 && info->GetInput<TensorImpl>(1)->GetShape()->GetDimCount() >= 3)
                type = ppl::common::DATATYPE_FLOAT16;
        }
        ...
        #else
            status = InferDefaultType(info, ppl::common::DATATYPE_FLOAT16);
        #endif
        } else {
