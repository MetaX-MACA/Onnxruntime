...
ppl::common::RetCode HardSigmoidKernel::DoExecute(KernelExecContext* ctx) {
    ...
    ppl::common::RetCode status =
        PPLCUDAUnaryHardSigmoidForwardImp(GetStream(), input->GetShape(), input->GetBufferPtr(), output->GetShape(),
                                        output->GetBufferPtr(), param_->alpha, param_->beta, input_quant.scale[0], output_quant.scale[0]);
    return status;
}
