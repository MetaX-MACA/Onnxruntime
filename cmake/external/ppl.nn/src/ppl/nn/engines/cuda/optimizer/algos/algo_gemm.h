...
public:
    GemmAlgorithm() {
    #ifdef PPLNN_USE_DNN
        std::set<dataformat_t> nd{DATAFORMAT_NDARRAY};
        gemm_formats_.emplace(DATAFORMAT_NDARRAY, nd);
    #endif
...
public:
    ...
    RetCode ModifyParam_PPL(ir::Node* node, OptKernelOptions& options);
    RetCode ModifyParam_BLAS(ir::Node* node, OptKernelOptions& options);
    RetCode ModifyParam_BLAS_INT8(ir::Node* node, OptKernelOptions& options);
