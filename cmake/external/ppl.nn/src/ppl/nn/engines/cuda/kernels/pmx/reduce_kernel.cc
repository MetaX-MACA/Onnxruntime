...
ppl::common::RetCode ReduceKernel::DoExecute(KernelExecContext* ctx) {
    ...
    if(param != ReduceParam::ReduceSum) {
        for(uint32_t i = 0; i < param_->axes.size(); i++)
            input_axes.push_back(param_->axes[i]);
    }else {
        auto axes_data = ctx->GetInput<TensorImpl>(1)->GetBufferPtr<int64_t>();
        if (axes_data != nullptr) {
            std::vector<int64_t> axes_data_host(ctx->GetInput<TensorImpl>(1)->GetShape()->GetDim(0));
            ctx->GetInput<TensorImpl>(1)->CopyToHost(axes_data_host.data());
            for(uint32_t i = 0; i < ctx->GetInput<TensorImpl>(1)->GetShape()->GetDim(0); i++) {
                input_axes.push_back(axes_data_host[i]);
            }
        }
    }

    if (input_axes.empty()) { // empty axes means reduce all dims
        if(!param_->noop_with_empty_axes){
            ...
            n_outer = accumulate(input_shape.GetDims(), input_shape.GetDims() + dim_count, n_outer,
                                std::multiplies<uint32_t>());
        }
        
    } else {
        std::vector<uint32_t> real_axis(input_axes.size());

        for (uint32_t i = 0; i < input_axes.size(); ++i) {
            real_axis[i] = (input_axes[i] + dim_count) % dim_count;
        ...
        n_outer = accumulate(input_shape.GetDims(), input_shape.GetDims() + real_axis[0], n_outer,
                             std::multiplies<uint32_t>());
        n_inner = accumulate(input_shape.GetDims() + real_axis[input_axes.size() - 1] + 1,
...
bool ReduceKernel::CanDoExecute(const KernelExecContext& ctx) const {
