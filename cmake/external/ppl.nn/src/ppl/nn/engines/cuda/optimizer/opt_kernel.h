...
struct OptKernelOptions final {
    ...
    OptKernelOptions(ir::Graph* graph, RuntimePartitionInfo* info, const utils::SharedResource* r,
                     std::map<edgeid_t, std::unique_ptr<TensorImpl>>* tensors, std::vector<CudaTensorQuant>* quants)
        : graph(graph), info(info), resource(r), tensors(tensors), quants(quants) {}
...
protected:
    ...
    static ppl::common::RetCode InferInheritedTypeForNoFusedInt8(InputOutputInfo* info) {
        TensorShape& in_shape = *info->GetInput<TensorImpl>(0)->GetShape();
        if (in_shape.GetDataType() == ppl::common::DATATYPE_UNKNOWN) {
            LOG(DEBUG) << "Input edge has unknown type.";
        }
    ...
    void GenericInferTypeAndFormat(InputOutputInfo* info) const {
        if(info->GetInputCount() != 0){
            ...
            for (uint32_t i = 0; i < info->GetOutputCount(); ++i) {
                ...
                auto data_format_force_to = (*common_param_.cuda_tensor_info)[edge_id].format_force_to;
                auto c_split_dims = (*common_param_.cuda_tensor_info)[edge_id].c_split_dims;
                ...
                tensor_shape.SetDataFormat(data_format == ppl::common::DATAFORMAT_UNKNOWN ? pre_shape->GetDataFormat()
                                                                                        : data_format);
                tensor_shape.SetDataFormatForceTo(data_format_force_to);
                tensor_shape.SetChannelSpilt(c_split_dims);
            }
        }
    }
