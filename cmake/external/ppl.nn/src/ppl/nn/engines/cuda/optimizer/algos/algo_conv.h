...
public:
    ConvAlgorithm() {
        ...
        conv_formats_.emplace(DATAFORMAT_NCHW8, nchw8);
        std::set<dataformat_t> nchw16{DATAFORMAT_NCHW16};
        conv_formats_.emplace(DATAFORMAT_NCHW16, nchw16);
    #endif
    }
    ...
    const std::map<dataformat_t, std::set<dataformat_t>> Getformats(const std::string& type_name) const override {
        ...
        if(gSupportNc1hwc0 == false){
            std::map<dataformat_t, std::set<dataformat_t>> conv_formats_tmp = conv_formats_;
            conv_formats_tmp.erase(DATAFORMAT_NCHW8);
            conv_formats_tmp.erase(DATAFORMAT_NCHW16);
            return conv_formats_tmp;
        }else{
            std::map<dataformat_t, std::set<dataformat_t>> conv_formats_tmp = conv_formats_;
            conv_formats_tmp.erase(DATAFORMAT_NHWC8);
            conv_formats_tmp.erase(DATAFORMAT_NHWC16);
            return conv_formats_tmp;
        }
    #endif
...
class TuringFMMAImpgemm final : public ConvAlgorithm {
public:
...
public:
    ...
    bool IsSupported(const ir::Node* node, const OptKernelOptions& options,
                     ppl::common::dataformat_t input_format) const override;
    double ExcuteTimer(const ir::Node* node, OptKernelOptions& options) override;
    void AddPreluSlopeDeQuantInput(ir::Node* node, OptKernelOptions& options, int fuse_index);
    RetCode ModifyParam(ir::Node* node, OptKernelOptions& options) override;
    void ReshapeOnEdges(const ir::Node* node, std::map<edgeid_t, std::unique_ptr<TensorImpl>>* tensors,
                        ppl::common::dataformat_t input_format, ppl::common::dataformat_t output_format) override;
