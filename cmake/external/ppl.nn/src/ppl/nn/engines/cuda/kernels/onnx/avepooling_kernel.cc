...
bool isGroupCall(int in_height, int in_width,int kernel_height,int kernel_width)
{
...
ppl::common::RetCode AvePoolingKernel::DoExecute(KernelExecContext* ctx) {
    ...
    if(input->GetShape()->GetDimCount() == 4){
        in_height    = input->GetShape()->GetDim(2);
        in_width     = input->GetShape()->GetDim(3);
    }else if(input->GetShape()->GetDimCount() == 3){
        in_height = input->GetShape()->GetDim(1);
        in_width = input->GetShape()->GetDim(2);
    }
    ...
    if (param_->global_pooling) {
        ...
        int32_t kernel_w;
        int32_t kernel_d;
        int32_t stride_h = param_->strides.size() >= 1 ? param_->strides[0] : 1;
        int32_t stride_w = param_->strides.size() >= 2 ? param_->strides[1] : 1;
        ...
        if(isGroupCall(in_height, in_width, kernel_h, kernel_w)&& param_->kernel_shape.size() != 3){
            return PPLCUDAGlobalAvePoolingForwardImp(GetStream(), input->GetShape(), input->GetBufferPtr(), output->GetShape(),
                                              output->GetBufferPtr(), input_quant.scale[0], output_quant.scale[0]);
        }
