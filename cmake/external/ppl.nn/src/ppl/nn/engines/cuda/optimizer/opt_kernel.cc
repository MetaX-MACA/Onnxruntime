...
RetCode CudaOptKernel::SetCommonParam(const OptKernelOptions& options) {
    ...
    for (uint32_t i = 0; i < node->GetOutputCount(); ++i) {
        ...
        temp_tensor.format_force_to = iter->second->GetShape()->GetDataFormatForceTo();
        temp_tensor.type = iter->second->GetShape()->GetDataType();
        temp_tensor.c_split_dims.clear();
        for(int i = 0; i < iter->second->GetShape()->GetChannelSpiltCount(); i++){
            temp_tensor.c_split_dims.push_back((iter->second->GetShape()->GetChannelSpilt())[i]);
        }
    }
    return RC_SUCCESS;
}
