...
bool GemmAlgorithm::IsSupported(const ir::Node* node, const OptKernelOptions& options,
    ...
    CudaGemmParam GemmParm = *(reinterpret_cast<CudaGemmParam*>(options.param));
    ...
    if (tensor0.GetDataType() != DATATYPE_FLOAT16 && tensor0.GetDataType() != DATATYPE_INT8 && tensor0.GetDataType() != DATATYPE_FLOAT32) {
        return false;
    }
#ifdef PPLNN_USE_DNN
    #ifndef PPLNN_USE_MACA
    if (tensor0.GetDataType() == DATATYPE_INT8) {
...
#endif
    if (input_format != DATAFORMAT_NHWC16 && input_format != DATAFORMAT_NHWC8 && input_format != DATAFORMAT_NDARRAY) {
        return false;
    }
    ...
    if (tensor0.GetDataType() == DATATYPE_FLOAT32 && input_format != DATAFORMAT_NDARRAY) {
        return false;
    }
    return true;
}
...
double GemmAlgorithm::ExcuteTimer(const ir::Node* node, OptKernelOptions& options) {
    ...
    if (attr_param_.extra_param.bias_term) {
        ...
        if(shape_in2.IsScalar())
            shape_in2.SetDimCount(1);
...
RetCode GemmAlgorithm::ModifyParam_PPL(ir::Node* node, OptKernelOptions& options) {
    this->attr_param_ = *(reinterpret_cast<CudaGemmParam*>(options.param));
    auto topo = options.graph->topo.get();
    auto data = options.graph->data.get();
    auto weight_edge = topo->GetEdge(node->GetInput(1));
    ...
    if (shape_in0.GetDataType() == DATATYPE_INT8)
    {
    ...
    if (weight_iter != data->constants.end() && // is a constant tensor and has not be loaded
        ...
        RuntimeConstantInfo weight_constat_info;
        {
            ...
            cudaMemsetAsync(weight_constat_info.GetBufferPtr(), 0, newshape.CalcBytesIncludingPadding(), stream);
        }
        ...
        reinterpret_cast<CudaGemmParam*>(options.param)->param.alpha = 1.0;
    ...
    if (bias_iter != data->constants.end() && // is a constant tensor and has not be loaded
        ...
        auto weight_preedge_id = weight_node->GetInput(0);
        ...
        const TensorShape& weightpreshape = *options.tensors->find(weight_preedge_id)->second->GetShape();
        bool is_bias_boardcast = false;
        void* bias_tmp = nullptr;
        if(preshape.IsScalar() || (preshape.GetDimCount() == 1 && preshape.GetDim(0) != weightpreshape.GetDim(0)))
        {
            if(preshape.IsScalar()){
                preshape.SetDimCount(1);
                postshape.SetDimCount(1);
            }
            preshape.SetDim(0, weightpreshape.GetDim(0));
            postshape.SetDim(0, weightpreshape.GetDim(0));
            is_bias_boardcast = true;
        }
        auto newshape = postshape;
        ...
        RuntimeConstantInfo bias_constat_info;
        {
            ...
            cudaMemsetAsync(bias_constat_info.GetBufferPtr(), 0, newshape.CalcBytesIncludingPadding(), stream);
        }

        if(is_bias_boardcast)
        {
            int ele_size = 4;
            if(preshape.GetDataType() ==  DATATYPE_FLOAT16)
                ele_size = 2;
            bias_tmp = malloc(preshape.GetDim(0) * ele_size);
            memcpy(bias_tmp, bias_iter->second.data.GetData(), ele_size);
            for(int i = 1; i < (preshape.GetDim(0)); i++)
                memcpy(bias_tmp + ele_size * i, bias_tmp, ele_size);
        ...
        auto status = PPLCUDAGemmModifyBias(stream, shape_in0.GetDataType(), &newshape,
                                            bias_constat_info.GetBufferDesc().addr, &param_kernel_);

        options.info->constants.emplace(preedge_id, std::move(bias_constat_info));
        reinterpret_cast<CudaGemmParam*>(options.param)->param.beta = 1.0;
        *options.tensors->find(preedge_id)->second->GetShape() = newshape;
        *options.tensors->find(postedge_id)->second->GetShape() = newshape;
        options.quants->at(preedge_id) = options.quants->at(postedge_id);
        options.quants->at(preedge_id).format = postshape.GetDataFormat();
...
RetCode GemmAlgorithm::ModifyParam_BLAS(ir::Node* node, OptKernelOptions& options) {
    this->attr_param_ = *(reinterpret_cast<CudaGemmParam*>(options.param));
    auto topo = options.graph->topo.get();
    auto data = options.graph->data.get();
    auto weight_edge = topo->GetEdge(node->GetInput(1));
    ...
    //param_kernel_.transB = 1;   // skip trans B matrix active
    param_kernel_.transB = !attr_param_.param.transB; // 0-transpose, 1-non
    ...
    if (weight_iter != data->constants.end() && // is a constant tensor and has not be loaded
        ...
        if ((shape_in0.GetDataType() == DATATYPE_FLOAT16 && (preshape.GetDataType() == DATATYPE_FLOAT16 || preshape.GetDataType() == DATATYPE_FLOAT32))
            || (shape_in0.GetDataType() == DATATYPE_FLOAT32 && preshape.GetDataType() != DATATYPE_INT8)) {
            status = options.device->GetDataConverter()->ConvertFromHost(
                &weight_constat_info.GetBufferDesc(), postshape, weight_iter->second.data.GetData(), preshape);
        } else if ((shape_in0.GetDataType() == DATATYPE_INT8 || preshape.GetDataType() == DATATYPE_INT8)
                   || (shape_in0.GetDataType() == DATATYPE_FLOAT32 && preshape.GetDataType() == DATATYPE_INT8)) {
            auto quants = options.quants;
            status = ((CudaDataConverter*)options.device->GetDataConverter())
                         ->ConvertFromHost(&weight_constat_info.GetBufferDesc(), postshape, (*quants)[postedge_id],
        ...
        if (shape_in0.GetDataType() == DATATYPE_FLOAT16 || shape_in0.GetDataType() == DATATYPE_FLOAT32) {
            status = PPLCUDAGemmModifyWeights(stream, &newshape, weight_constat_info.GetBufferPtr(),
                                              temp_weight_buffer.addr, &param_kernel_);
        } else if (shape_in0.GetDataType() == DATATYPE_INT8) {
            status = PPLCUDAGemmModifyWeightsInt8(stream, &newshape, weight_constat_info.GetBufferPtr(),
        ...
        weight_constat_info.Reshape(newshape); // 'PPLCUDAGemmModifyWeights' may transpose weights
        //reinterpret_cast<CudaGemmParam*>(options.param)->param.transB = 1;
        reinterpret_cast<CudaGemmParam*>(options.param)->param.transB = 0;
        reinterpret_cast<CudaGemmParam*>(options.param)->param.alpha = 1.0;
        options.info->constants.emplace(preedge_id, std::move(weight_constat_info));
        *options.tensors->find(preedge_id)->second->GetShape() = newshape;
        *options.tensors->find(postedge_id)->second->GetShape() = newshape;
    ...
    if (bias_iter != data->constants.end() && // is a constant tensor and has not be loaded
        ...
        bool is_bias_boardcast = false;
        void* bias_tmp = nullptr;
        //int bias_real_len = attr_param_.param.transB ?  weightpreshape.GetDim(0) :  weightpreshape.GetDim(1);
        int bias_real_len = weightpreshape.GetDim(1);
        if(preshape.IsScalar() || (preshape.GetDimCount() == 1 && preshape.GetDim(0) != bias_real_len))
        {
            if(preshape.IsScalar()){
                preshape.SetDimCount(1);
                postshape.SetDimCount(1);
            }
            preshape.SetDim(0, bias_real_len);
            postshape.SetDim(0, bias_real_len);
            is_bias_boardcast = true;
        }
        auto newshape = postshape;
...
RetCode GemmAlgorithm::ModifyParam_BLAS_INT8(ir::Node* node, OptKernelOptions& options) {
    this->attr_param_ = *(reinterpret_cast<CudaGemmParam*>(options.param));
    auto stream = options.device->GetStream();
    auto topo = options.graph->topo.get();
    auto data = options.graph->data.get();
    ...
    if (shape_in0.GetDataType() == DATATYPE_INT8)
    {
        ...
        float alpha = attr_param_.param.alpha;

        auto total_size = weight_quant.scale.size();
        std::vector<float> scales(total_size);
        for (int i = 0; i < total_size; i++) {
            scales[i] = alpha * input_quant.scale[0] * weight_quant.scale[i] / output_quant.scale[0];
        }
    ...
    auto bias_node = topo->GetNode(bias_edge->GetProducer());
    auto bias_iter = data->constants.find(bias_node->GetInput(0));
    const TensorShape& shape_in2 = *options.tensors->find(node->GetInput(2))->second->GetShape();
    ...
    if (bias_iter != data->constants.end() && // is a constant tensor and has not be loaded
        ...
        RuntimeConstantInfo bias_constat_info;
        {
            ...
            // if(shape_in0.GetDataType() == DATATYPE_INT8) {
            //     newshape = shape_out;
            //     newshape.SetDataType(ppl::common::DATATYPE_FLOAT32);
            // }
            auto status = options.device->Realloc(newshape, &buffer);
            if (status != RC_SUCCESS) {
                LOG(ERROR) << "alloc buffer for constant failed: " << GetRetCodeStr(status);
        ...
        float* bias_ptr = (float*)bias_iter->second.data.GetData();
        auto total_size = shape_in2.CalcElementsIncludingPadding();
        auto& output_quant = options.quants->at(node->GetOutput(0));
        float beta = attr_param_.param.beta;
        std::vector<float> bias(total_size);
        for (int i = 0; i < total_size; i++) {
            bias[i] = beta * bias_ptr[i] / output_quant.scale[0];
        }
        ...
        *options.tensors->find(bias_edge_id)->second->GetShape() = preshape;
        options.quants->resize(topo->GetCurrentEdgeIdBound());
...
RetCode GemmAlgorithm::ModifyParam(ir::Node* node, OptKernelOptions& options) {
    ...
    if (tensor0.GetDataType() == DATATYPE_INT8) {
    #ifdef PPLNN_USE_MACA
        return ModifyParam_BLAS_INT8(node, options);
    #else
        return ModifyParam_PPL(node, options);
    #endif
    }
    return ModifyParam_BLAS(node, options);
#else
    return ModifyParam_PPL(node, options);
#endif
}
