...
#include "ppl/nn/engines/cuda/kernels/onnx/isnan_kernel.h"
using namespace std;
using namespace ppl::common;

namespace ppl { namespace nn { namespace cuda {

RetCode IsnanOp::Init(const OptKernelOptions& options) {
    return RC_SUCCESS;
}

IsnanOp::IsnanOp(const ir::Node* node) : CudaOptKernel(node) {
    infer_type_func_ = [](InputOutputInfo* info, std::vector<CudaTensorQuant>* quant, datatype_t type) -> RetCode {
        ...
        {
            LOG(ERROR) << "IsnanOp input datatye must be : FLOAT16/FLOAT32/FLOAT64";
...
RetCode IsnanOp::Finalize(const OptKernelOptions& options) {
...
KernelImpl* IsnanOp::CreateKernelImpl() const {
    return CreateKernelImplWithoutParam<IsnanKernel>();
}
