...
#ifndef _ST_HPC_PPL_NN_ENGINES_CUDA_OPTIMIZER_ALGOS_ALGO_PAD_H_
#define _ST_HPC_PPL_NN_ENGINES_CUDA_OPTIMIZER_ALGOS_ALGO_PAD_H_
...
class PadAlgorithm : public Algorithm {
public:
    PadAlgorithm() {
        std::set<dataformat_t> ndarray{DATAFORMAT_NDARRAY};
        pad_formats_.emplace(DATAFORMAT_NDARRAY, ndarray);
    #ifdef PPLNN_USE_DNN
        // std::set<dataformat_t> nhwc{DATAFORMAT_NHWC};
        // pad_formats_.emplace(DATAFORMAT_NHWC, nhwc);
        // std::set<dataformat_t> nhwc8{DATAFORMAT_NHWC8};
        // pad_formats_.emplace(DATAFORMAT_NHWC8, nhwc8);
        std::set<dataformat_t> nhwc8{DATAFORMAT_NHWC8};
        pad_formats_.emplace(DATAFORMAT_NHWC8, nhwc8);
        std::set<dataformat_t> nhwc16{DATAFORMAT_NHWC16};
        pad_formats_.emplace(DATAFORMAT_NHWC16, nhwc16);
        std::set<dataformat_t> nchw8{DATAFORMAT_NCHW8};
        pad_formats_.emplace(DATAFORMAT_NCHW8, nchw8);
        std::set<dataformat_t> nchw16{DATAFORMAT_NCHW16};
        pad_formats_.emplace(DATAFORMAT_NCHW16, nchw16);
    #endif
    }
    ...
    bool IsSupported(const ir::Node* node, const OptKernelOptions& options, dataformat_t input_format) const override{
        ...
        if(env_ptr != nullptr) {
            ...
            if (input_format == DATAFORMAT_NCHW8 && tensor0.GetDataType() == DATATYPE_FLOAT16) {
                return true;
            }
            if (input_format == DATAFORMAT_NDARRAY || input_format == DATAFORMAT_NHWC8) {
                return true;
            }
            return false;
        }
    ...
    const std::map<dataformat_t, std::set<dataformat_t>> Getformats(const std::string& type_name) const override {
        ...
        if(gSupportNc1hwc0 == false){
            std::map<dataformat_t, std::set<dataformat_t>> pad_formats_tmp = pad_formats_;
            pad_formats_tmp.erase(DATAFORMAT_NCHW8);
            pad_formats_tmp.erase(DATAFORMAT_NCHW16);
            return pad_formats_tmp;
        }else{
            std::map<dataformat_t, std::set<dataformat_t>> pad_formats_tmp = pad_formats_;
            //pad_formats_tmp.erase(DATAFORMAT_NHWC8);
            pad_formats_tmp.erase(DATAFORMAT_NHWC16);
            return pad_formats_tmp;
        }
    #endif
        return pad_formats_;
...
private:
    std::map<dataformat_t, std::set<dataformat_t>> pad_formats_;
};
