...
ppl::common::RetCode PadKernel::DoExecute(KernelExecContext* ctx) {
    ...
    uint32_t status = ppl::common::RC_SUCCESS;
    ...
    if(PadKernelParam::PAD_MODE_CONSTANT == kernel_param.mode){
        if (ctx->GetInputCount() == 3 && ctx->GetNode()->GetInput(2) != INVALID_EDGEID) {
            auto constant_data = ctx->GetInput<TensorImpl>(2);
            status = constant_data->CopyToHost(&(kernel_param.constant_value));
            if (status != ppl::common::RC_SUCCESS) {
                LOG(ERROR) << "Copy constant value failed: " << ppl::common::GetRetCodeStr(status);
                return status;
            }
        } else {
            kernel_param.constant_value = 0;
        }
