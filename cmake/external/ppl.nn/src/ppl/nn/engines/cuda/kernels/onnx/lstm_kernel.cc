...
#endif
    // TODO: Only supported all sequences in the batch have length seq_length
    if (seq_lens_ptr){
        int seq_len = X_shape->GetDim(0);
        int batch   = X_shape->GetDim(1);
        int tmp[batch];
        sequence_lens->ConvertToHost(&tmp, *sequence_lens->GetShape());
        for (int32_t i = 0; i < batch; ++i){
            if (tmp[i] != seq_len){
                LOG(ERROR) << " NOT support invalid value sequence_lens";
                return  ppl::common::RC_UNSUPPORTED;
            }
        }
        seq_lens_ptr = nullptr;
    }
