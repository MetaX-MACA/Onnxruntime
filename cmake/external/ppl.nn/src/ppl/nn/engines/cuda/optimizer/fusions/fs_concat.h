...
class ConcatBridgeFusion : public MacaFusion {
public:
    ConcatBridgeFusion(){
        fusion_tree_.node_types = {"Concat", "Bridge"};
        fusion_tree_.connect_infos = {{0,1}};
        fusion_type_ = "FusedConcatBridge";
    };
...
class ConcatFusion_Set : public Fusion{
public:
    ppl::common::RetCode FuseNode(ir::Node* node, bool reliable, const OptKernelOptions& options) override;
private:
    ConcatFusion conv_concat_;
    ConcatBridgeFusion concat_bridge_;
