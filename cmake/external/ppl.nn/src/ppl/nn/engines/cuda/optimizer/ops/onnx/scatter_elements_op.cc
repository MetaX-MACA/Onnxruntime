...
ScatterElementsOp::ScatterElementsOp(const ir::Node* node) : CudaOptKernel(node) {
    ...
    infer_type_func_ = [](InputOutputInfo* info, std::vector<CudaTensorQuant>* quant, datatype_t type) -> RetCode {
        ...
        auto shape_data = info->GetInput<TensorImpl>(0)->GetShape();
        auto shape_update = info->GetInput<TensorImpl>(2)->GetShape();
        shape_update->SetDataType(shape_data->GetDataType());
        return status;
    };
