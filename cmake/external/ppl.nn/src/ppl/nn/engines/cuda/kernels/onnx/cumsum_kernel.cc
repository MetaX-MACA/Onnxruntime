...
ppl::common::RetCode CumSumKernel::DoExecute(KernelExecContext* ctx) {
    ...
    if(axis < 0)
    {
        axis += input->GetShape()->GetDimCount();
    }
   
    ppl::common::RetCode status = PPLCUDACumsumForwardImp(GetStream(), axis, param_->reverse, param_->exclusive, input->GetShape(), input->GetBufferPtr(),
                                                            output->GetBufferPtr());
