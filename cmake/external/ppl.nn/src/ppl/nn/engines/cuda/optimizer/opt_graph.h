...
private:
    ppl::common::RetCode graphPreOptProcess(const utils::SharedResource& resource);
    ppl::common::RetCode InitKernels();
    ppl::common::RetCode findGraphInputConv(const utils::SharedResource& resource);
    ...
    ppl::common::RetCode AdjustGemmInOutFormat(const utils::SharedResource& resource, CudaDevice* device);
    ppl::common::RetCode DeleteBridgeKernels();
    ppl::common::RetCode DeleteCastKernels();
    int32_t LastLegalNodeIndex();
    void UpdateTopologicalSort();
    void DumpGraphInfo(std::string stage);
