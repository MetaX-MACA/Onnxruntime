...
double BridgeAlgorithm::ExcuteTimer(const ir::Node* node, OptKernelOptions& options) {
    ...
    auto prostedge_id = node->GetOutput(0);
    const TensorShape& preshape = *options.tensors->find(preedge_id)->second.get()->GetShape();
    const TensorShape& postshape = *options.tensors->find(prostedge_id)->second.get()->GetShape();
    double timer = 0.0;

    if (input_format_ != output_format_ || preshape.GetDataType() != postshape.GetDataType()) {
