...
CudaDevice::~CudaDevice() {
    if (stream_ && (external_stream_==false)) {
    ...
    if (mcblaslt_handle_) {
        mcblasLtDestroy(mcblaslt_handle_);
    }
...
RetCode CudaDevice::Init(int device_id, void* external_stream_queue) {
    ...
    if(external_stream_queue){
        stream_ = (static_cast<std::queue<cudaStream_t>*>(external_stream_queue))->front();
        (static_cast<std::queue<cudaStream_t>*>(external_stream_queue))->pop();
...
#ifdef PPLNN_USE_MACA
    if (!mcblaslt_handle_) {
        mcblasLtCreate(&mcblaslt_handle_);
    }
#endif//PPLNN_USE_MACA
#endif
