...
ppl::common::RetCode SliceKernel::DoExecute(KernelExecContext* ctx) {
    ...
    if (!kernel_param.is_constant_starts) { // starts
    ...
    if (!kernel_param.is_constant_ends){ // ends
    ...
    for (int it = 0; it < kernel_param.axes_num; ++it) {
        ...
        kernel_param_fix.axes[it] = (axis + dim_count) % dim_count;
        ...
        if(kernel_param.steps[it] < 0)
        {
            start_val = (start_val >= cur_dim_size) ? (cur_dim_size - 1) : start_val;
            end_val = (end_val >= cur_dim_size) ? (cur_dim_size - 1) : end_val;
        }
    ...
    if (input->GetEdge()->CalcConsumerCount() == 1 && input->GetType() == TENSORTYPE_NORMAL &&
        ...
        status = PPLCUDASliceForwardImp(GetStream(), kernel_param_fix, input->GetShape(), input->GetBufferPtr(),
