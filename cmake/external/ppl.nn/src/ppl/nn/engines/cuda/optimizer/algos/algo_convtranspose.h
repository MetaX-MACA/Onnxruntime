...
public:
    ConvTransposeAlgorithm() {
        ...
        conv_transpose_formats_.emplace(DATAFORMAT_NHWC, nhwc);
        ...
        conv_transpose_formats_.emplace(DATAFORMAT_NCHW8, nchw8);
    #endif
    }
    ...
    const std::map<dataformat_t, std::set<dataformat_t>> Getformats(const std::string& type_name) const override {
        ...
        if(gSupportNc1hwc0 == false){
            std::map<dataformat_t, std::set<dataformat_t>> conv_transpose_formats_tmp = conv_transpose_formats_;
            conv_transpose_formats_tmp.erase(DATAFORMAT_NCHW8);
            return conv_transpose_formats_tmp;
        }else{
            std::map<dataformat_t, std::set<dataformat_t>> conv_transpose_formats_tmp = conv_transpose_formats_;
            conv_transpose_formats_tmp.erase(DATAFORMAT_NHWC8);
            return conv_transpose_formats_tmp;
        }
    #endif
