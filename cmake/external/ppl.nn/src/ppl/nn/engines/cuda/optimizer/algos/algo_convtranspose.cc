...
bool ConvTransposeAlgorithm::IsSupported(const ir::Node* node, const OptKernelOptions& options,
    ...
    if (tensor0.GetDataType() != ppl::common::DATATYPE_FLOAT16 && tensor0.GetDataType() != ppl::common::DATATYPE_FLOAT32) {
        return false;
    }
#ifdef PPLNN_USE_DNN
    if(gSupportNc1hwc0){
    ...
    if (tensor0.GetDataType() == ppl::common::DATATYPE_FLOAT16) {
        if(group > 1 && !(group == tensor1.GetDim(0))){
    ...
    if (input_format == DATAFORMAT_NDARRAY && tensor0.GetDataType() == ppl::common::DATATYPE_FLOAT32) {
        return true;
    }
    return false;
}
...
RetCode ConvTransposeAlgorithm::ModifyParam(ir::Node* node, OptKernelOptions& options) {
    ...
    if (weight_iter != data->constants.end() && // is a constant tensor and has not be loaded
        ...
        RuntimeConstantInfo weight_constat_info;
        {
            ...
            if(shape_in0.GetDataType() == DATATYPE_FLOAT16 && attr_param_.param.group > 1){
    ...
    if (bias_iter != data->constants.end() && // is a constant tensor and has not be loaded
        ...
        auto newshape = postshape;
        if(shape_in0.GetDataType() == DATATYPE_FLOAT16 && shape_in0.GetDataFormat() == DATAFORMAT_NHWC8)
            newshape.SetDim(0, (newshape.GetDim(0) + 7) / 8 * 8);
    ...
    return RC_SUCCESS;
#else
    return RC_SUCCESS; // Convtranspose weight为常量输出结果不对问题临时修改，先让有此算子的模型先能够正确跑起来，后续需解决此问题，见JIRA C500-19744
