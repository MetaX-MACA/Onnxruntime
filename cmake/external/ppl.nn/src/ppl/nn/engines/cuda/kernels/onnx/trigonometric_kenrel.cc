...
#define ExecuteInStant(Operator) \
ppl::common::RetCode Operator##Kernel::DoExecute(KernelExecContext* ctx) { \
    ...
    ppl::common::RetCode status = PPLCUDAUnary##Operator##ForwardImp(GetStream(), input->GetShape(), input->GetBufferPtr(), \ 
...
ExecuteInStant(Acos)
ExecuteInStant(Atan)
ExecuteInStant(Asinh)
ExecuteInStant(Acosh)
ExecuteInStant(Atanh)
