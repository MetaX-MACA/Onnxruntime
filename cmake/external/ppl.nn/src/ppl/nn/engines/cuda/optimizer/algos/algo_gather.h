...
#ifndef _ST_HPC_PPL_NN_ENGINES_CUDA_OPTIMIZER_ALGOS_ALGO_GATHER_H_
#define _ST_HPC_PPL_NN_ENGINES_CUDA_OPTIMIZER_ALGOS_ALGO_GATHER_H_
...
public:
    GatherAlgorithm() {
        ...
        gather_formats_.emplace(DATAFORMAT_NDARRAY, ndarray);
    #ifdef PPLNN_USE_DNN
        // std::set<dataformat_t> nhwc{DATAFORMAT_NHWC};
        // gather_formats_.emplace(DATAFORMAT_NHWC, nhwc);
        std::set<dataformat_t> nhwc8{DATAFORMAT_NHWC8};
        gather_formats_.emplace(DATAFORMAT_NHWC8, nhwc8);
        std::set<dataformat_t> nhwc16{DATAFORMAT_NHWC16};
        gather_formats_.emplace(DATAFORMAT_NHWC16, nhwc16);
        std::set<dataformat_t> nchw8{DATAFORMAT_NCHW8};
        gather_formats_.emplace(DATAFORMAT_NCHW8, nchw8);
        std::set<dataformat_t> nchw16{DATAFORMAT_NCHW16};
        gather_formats_.emplace(DATAFORMAT_NCHW16, nchw16);
    #endif
    }
    ...
    bool IsSupported(const ir::Node* node, const OptKernelOptions& options, dataformat_t input_format) const override{
        ...
        if(env_ptr != nullptr) {
            ...
            if(!isFormatSupport(op_format_str))
                LOG(WARNING) << "Gather MACART_OP_FORMAT only support ND/NHWC16/NHWC8/NCHW8/NCHW16, but set:" << env_ptr;
            return false;
        } else {
            ...
            if ((input_format == DATAFORMAT_NHWC16 || input_format == DATAFORMAT_NCHW16) && tensor0.GetDataType() == DATATYPE_INT8) {
                auto graph_data = options.graph->data.get();
                auto param_ref = graph_data->attrs.find(node->GetId());
                if (param_ref == graph_data->attrs.end()) {
                    return false;
                }
                ppl::nn::onnx::GatherParam param = *((const ppl::nn::onnx::GatherParam*)param_ref->second.get());
                if (tensor0.GetDimCount() == 4 && param.axis == 1 &&
                        tensor0.GetDim(1) <= 4096 && tensor1.CalcElementsIncludingPadding() <= 1024) {
                    return true;
                }
                return false;
            }
            if ((input_format == DATAFORMAT_NHWC8 || input_format == DATAFORMAT_NCHW8) && tensor0.GetDataType() == DATATYPE_FLOAT16) {
                auto graph_data = options.graph->data.get();
                auto param_ref = graph_data->attrs.find(node->GetId());
                if (param_ref == graph_data->attrs.end()) {
                    return false;
                ...
                ppl::nn::onnx::GatherParam param = *((const ppl::nn::onnx::GatherParam*)param_ref->second.get());
                if (tensor0.GetDimCount() == 4 && param.axis == 1 &&
                        tensor0.GetDim(1) <= 2048 && tensor1.CalcElementsIncludingPadding() <= 1024) {
                    return true;
                }
                return false;
            }
    ...
    const std::map<dataformat_t, std::set<dataformat_t>> Getformats(const std::string& type_name) const override {
        ...
        if(gSupportNc1hwc0 == false){
            std::map<dataformat_t, std::set<dataformat_t>> gather_formats_tmp = gather_formats_;
            gather_formats_tmp.erase(DATAFORMAT_NCHW8);
            gather_formats_tmp.erase(DATAFORMAT_NCHW16);
            return gather_formats_tmp;
        }else{
            std::map<dataformat_t, std::set<dataformat_t>> gather_formats_tmp = gather_formats_;
            gather_formats_tmp.erase(DATAFORMAT_NHWC8);
            gather_formats_tmp.erase(DATAFORMAT_NHWC16);
            return gather_formats_tmp;
        }
    #endif
        return gather_formats_;
    }
...
private:
    std::map<dataformat_t, std::set<dataformat_t>> gather_formats_;
};
