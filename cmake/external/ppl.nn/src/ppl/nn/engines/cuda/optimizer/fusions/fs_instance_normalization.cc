...
RetCode InstanceNormalizationFusion::FuseNode(ir::Node* node, bool reliable, const OptKernelOptions& options) {
    ...
    if (CanFuse(nextnode)) {
        ...
        if(node->GetType().name == "InstanceNormalization"){
            CudaInstanceNormalizationParam* param = (CudaInstanceNormalizationParam*)opt_kernel->GetParam();
            param->extra_param.has_relu = true;
        }
        if(node->GetType().name == "GroupNormalization"){
            CudaGroupNormalizationParam* param = (CudaGroupNormalizationParam*)opt_kernel->GetParam();
            param->extra_param.has_relu = true;
        }
