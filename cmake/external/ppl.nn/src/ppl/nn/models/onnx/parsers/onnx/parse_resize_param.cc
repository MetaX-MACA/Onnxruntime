...
RetCode ParseResizeParam(const ::onnx::NodeProto& pb_node, const ParamParserExtraArgs& args, ir::Node*, ir::Attr* arg) {
    ...
    if(param->coord_trans_mode == 4 || param->coord_trans_mode == 0){
        utils::GetNodeAttr(pb_node,"nearest_mode", &str, "round_prefer_floor");
    }else if(param->coord_trans_mode == 3)
    {
        utils::GetNodeAttr(pb_node,"nearest_mode", &str, "round_prefer_ceil");
    }else{
        utils::GetNodeAttr(pb_node, "nearest_mode", &str, "floor");
    }
