...
RetCode ParseHardSigmoidParam(const ::onnx::NodeProto& pb_node, const ParamParserExtraArgs& args, ir::Node* node,
                              ir::Attr* arg) {
    auto param = static_cast<HardSigmoidParam*>(arg);
    if(node->GetType().name == "HardSwish"){
        utils::GetNodeAttr(pb_node, "alpha", &param->alpha, 1.0f / 6.0f);
