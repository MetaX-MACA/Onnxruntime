...
static bool SaveOutputsOneByOne(const Runtime* runtime) {
    for (uint32_t c = 0; c < runtime->GetOutputCount(); ++c) {
        ...
        string tensor_name(t->GetName());
        for (int i = 0; i < tensor_name.length(); i++)
            if(tensor_name[i] == '/') tensor_name[i] = '-';
        string out_file_name = g_flag_save_data_dir + "/pplnn_output-" + tensor_name + ".dat";
