...
if(onnxruntime_USE_PPL)
  set(PROVIDERS_MACA onnxruntime_providers_maca)
endif()
...
if (onnxruntime_USE_PPL)
  set(CMAKE_CXX_FLAGS "${CMAKE_CXX_FLAGS} -Wno-unused-variable -Wno-unused-parameter")
  add_definitions(-DUSE_PPL=1)

  set(PPLNN_USE_X86_64  OFF)
  set(PPLNN_USE_CUDA  ON)
  set(PPLNN_BUILD_TESTS  OFF)
  set(PPLNN_BUILD_TOOLS  ON)
  set(PPLNN_BUILD_SAMPLES  OFF)
  set(PPLNN_INSTALL  OFF)
  set(PPLNN_USE_QDQ ON)
  set(HPCC_DEPS_DIR ${CMAKE_BINARY_DIR}/external/ppl.nn/deps)

  if(onnxruntime_USE_DNN)
    set(PPLNN_USE_DNN ON)
  endif()

  if(onnxruntime_USE_MACA)
    set(PPLNN_USE_MACA ON)
    set(PPLNN_ENABLE_CUDA_JIT  OFF)
  else()
    set(PPLNN_ENABLE_CUDA_JIT  ON)
  endif()
  ...
  add_subdirectory(${PROJECT_SOURCE_DIR}/external/ppl.nn)

  file(GLOB_RECURSE
    onnxruntime_providers_maca_cc_srcs CONFIGURE_DEPENDS
    "${ONNXRUNTIME_ROOT}/core/providers/maca/*.h"
    "${ONNXRUNTIME_ROOT}/core/providers/maca/*.cc"
    # "${ONNXRUNTIME_ROOT}/core/providers/shared_library/*.h"
    # "${ONNXRUNTIME_ROOT}/core/providers/shared_library/*.cc"
  )
  source_group(TREE ${ONNXRUNTIME_ROOT}/core FILES ${onnxruntime_providers_maca_cc_srcs})
  onnxruntime_add_static_library(onnxruntime_providers_maca ${onnxruntime_providers_maca_cc_srcs})
  onnxruntime_add_include_to_target(onnxruntime_providers_maca
    onnxruntime_common onnxruntime_framework onnx onnx_proto protobuf::libprotobuf-lite flatbuffers
  )
  add_dependencies(onnxruntime_providers_maca ${onnxruntime_EXTERNAL_DEPENDENCIES})
  set_target_properties(onnxruntime_providers_maca PROPERTIES FOLDER "ONNXRuntime")
  target_include_directories(onnxruntime_providers_maca PRIVATE
    ${ONNXRUNTIME_ROOT}
    ${PROJECT_SOURCE_DIR}/external/ppl.nn/include
    ${PROJECT_SOURCE_DIR}/external/ppl.nn/src
  )

  target_link_libraries(onnxruntime_providers_maca PRIVATE
    # pplnn_basic_static
    # pplnn_cuda_static
    # pplnn_x86_static
    # pplnn_onnx_static
    # pplkernelcuda_static
    # ${PPLNN_LIBRARIES}
    pplnn_static
  )
  set_target_properties(onnxruntime_providers_maca PROPERTIES LINKER_LANGUAGE CXX)
  # add_dependencies(onnxruntime_providers_ppl onnxruntime_providers_shared ${onnxruntime_EXTERNAL_DEPENDENCIES} ${onnxruntime_tvm_dependencies})
  # target_link_libraries(onnxruntime_providers_ppl PRIVATE cudart ${ONNXRUNTIME_PROVIDERS_SHARED})
  # target_include_directories(onnxruntime_providers_ppl PRIVATE ${ONNXRUNTIME_ROOT} ${CMAKE_CURRENT_BINARY_DIR} ${eigen_INCLUDE_DIRS} ${TVM_INCLUDES} PUBLIC ${CMAKE_CUDA_TOOLKIT_INCLUDE_DIRECTORIES})
  # # ${CMAKE_CURRENT_BINARY_DIR} is so that #include "onnxruntime_config.h" inside tensor_shape.h is found
  # set_target_properties(onnxruntime_providers_ppl PROPERTIES LINKER_LANGUAGE CUDA)
  # set_target_properties(onnxruntime_providers_ppl PROPERTIES FOLDER "ONNXRuntime")
endif()
